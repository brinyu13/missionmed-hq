...
if ( ! function_exists( 'mm_drj_drills_access_defaults' ) ) {
	...
	function mm_drj_drills_access_defaults() {
		return array(
			...
			'full_access_capabilities' => array( 'missionmed_access_arena_pro', 'missionmed_access_arena_full' ),
			'protected_daily_routes' => array( '/daily', '/drills', '/daily-drills-v3', '/drills-v3' ),
...
if ( ! function_exists( 'mm_drj_drills_access_config' ) ) {
	...
	function mm_drj_drills_access_config() {
		...
		$config['full_access_capabilities'] = mm_drj_drills_access_normalize_mode_list( $config['full_access_capabilities'] ?? array() );
		$config['protected_daily_routes'] = mm_drj_drills_access_normalize_route_list( $config['protected_daily_routes'] ?? array() );
...
if ( ! function_exists( 'mm_drj_drills_access_user_is_restricted' ) ) {
	function mm_drj_drills_access_user_has_full_access( $user_id ) {
		$user_id = absint( $user_id );
		if ( $user_id <= 0 || user_can( $user_id, 'manage_options' ) ) {
			return $user_id > 0;
		}
		foreach ( (array) mm_drj_drills_access_config()['full_access_capabilities'] as $capability ) {
			if ( user_can( $user_id, $capability ) ) {
				return true;
			}
		}
		return (bool) apply_filters( 'missionmed_drj_drills_full_access', false, $user_id );
	}
	...
	function mm_drj_drills_access_user_is_restricted( $user_id = 0 ) {
		...
		if ( mm_drj_drills_access_user_has_full_access( $user_id ) ) {
			return false;
		}
...
if ( ! function_exists( 'mm_drj_drills_access_positive_daily_route_guard' ) ) {
	function mm_drj_drills_access_positive_daily_route_guard() {
		$config = mm_drj_drills_access_config();
		$path = mm_drj_drills_access_request_path();
		$protected = false;
		foreach ( (array) $config['protected_daily_routes'] as $route ) {
			if ( $path === $route || 0 === strpos( $path, rtrim( $route, '/' ) . '/' ) ) {
				$protected = true;
				break;
			}
		}
		...
		$legacy_arena = function_exists( 'sfwd_lms_has_access' ) && sfwd_lms_has_access( absint( $config['matrix_course_id'] ), $user_id );
		if ( $legacy_arena || mm_drj_drills_access_user_has_full_access( $user_id ) || mm_drj_drills_access_user_is_restricted( $user_id ) ) {
			return;
		}
		status_header( 403 );
		nocache_headers();
		header( 'X-MissionMed-Access-Layer: daily-rounds-entitlement-required' );
		wp_die( esc_html__( 'An active Daily Rounds entitlement is required.', 'missionmed' ), esc_html__( 'Daily Rounds access required', 'missionmed' ), array( 'response' => 403 ) );
	}
}
add_action( 'parse_request', 'mm_drj_drills_access_positive_daily_route_guard', -90 );
