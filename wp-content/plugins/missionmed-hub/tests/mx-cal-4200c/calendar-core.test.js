...
test('Scheduler degradation never creates an automatic retry loop', async () => {
	const scheduledDelays = [];
	const app = {
		profile: { is_admin: true },
		api: {
			base: '/wp-json/mmed/v1',
			request: (endpoint) => Promise.resolve(
	...
	const core = loadCore({
		...
		clearTimeout,
		fetch: () => Promise.reject(new Error('Scheduler offline'))
	});
	const calendar = core.create(app);
	await calendar.start();
	...
	assert.equal(calendar.state.schedulerStatus, 'degraded');
	assert.equal(
		scheduledDelays.includes(10000),
		false,
		'a failed recursive Scheduler auth path must not poll PHP every 10 seconds'
	);
	calendar.destroy();
});
