...
class MMED_REST_API {
	...
	/**
	 * Account-backed Appointments presentation preference.
	 */
	const APPOINTMENTS_EXPERIENCE_META_KEY = '_mmed_appointments_experience';

	/**
	 * Bounded audit ledger for the admin-only emergency presentation override.
	 */
	const APPOINTMENTS_FORCE_CLASSIC_AUDIT_OPTION = 'mmed_appointments_force_classic_audit';
	...
	public static function register_routes() {
		...
		register_rest_route(
			self::NAMESPACE,
			'/me/appointments-experience',
			array(
				array(
					'methods'             => WP_REST_Server::READABLE,
					'callback'            => array( __CLASS__, 'get_appointments_experience' ),
					'permission_callback' => array( __CLASS__, 'can_access' ),
				),
				array(
					'methods'             => WP_REST_Server::CREATABLE,
					'callback'            => array( __CLASS__, 'save_appointments_experience' ),
					'permission_callback' => array( __CLASS__, 'can_access' ),
					'args'                => array(
						'experience' => array(
							'required'          => true,
							'sanitize_callback' => 'sanitize_key',
							'validate_callback' => array( __CLASS__, 'is_valid_appointments_experience' ),
						),
					),
				),
			)
		...
		register_rest_route(
			self::NAMESPACE,
			'/admin/appointments-experience',
			array(
				array(
					'methods'             => WP_REST_Server::READABLE,
					'callback'            => array( __CLASS__, 'get_admin_appointments_experience' ),
					'permission_callback' => array( __CLASS__, 'can_manage' ),
				),
				array(
					'methods'             => WP_REST_Server::CREATABLE,
					'callback'            => array( __CLASS__, 'save_admin_appointments_experience' ),
					'permission_callback' => array( __CLASS__, 'can_manage' ),
					'args'                => array(
						'force_classic' => array(
							'required'          => true,
	...
	/**
	 * Validate an Appointments presentation preference.
	 *
	 * @param mixed $value Candidate value.
	 * @return bool
	 */
	public static function is_valid_appointments_experience( $value ) {
		return in_array( sanitize_key( $value ), array( 'classic', 'storyforge' ), true );
	}

	/**
	 * Return the current account preference and server-authorized effective value.
	 *
	 * @return WP_REST_Response
	 */
	public static function get_appointments_experience() {
		return new WP_REST_Response( self::appointments_experience_payload( get_current_user_id() ), 200 );
	}

	/**
	 * Persist the current account's Appointments presentation preference.
	 *
	 * The emergency Force Classic flag only changes the effective presentation;
	 * it never overwrites the student's stored preference.
	 *
	 * @param WP_REST_Request $request REST request.
	 * @return WP_REST_Response|WP_Error
	 */
	public static function save_appointments_experience( $request ) {
		$experience = sanitize_key( $request->get_param( 'experience' ) );
		if ( ! self::is_valid_appointments_experience( $experience ) ) {
			return new WP_Error( 'mmed_invalid_appointments_experience', 'Choose Classic or StoryForge.', array( 'status' => 400 ) );
		}

		update_user_meta( get_current_user_id(), self::APPOINTMENTS_EXPERIENCE_META_KEY, $experience );

		return new WP_REST_Response( self::appointments_experience_payload( get_current_user_id() ), 200 );
	}

	/**
	 * Return admin-visible Force Classic state and its bounded audit trail.
	 *
	 * @return WP_REST_Response
	 */
	public static function get_admin_appointments_experience() {
		return new WP_REST_Response( self::admin_appointments_experience_payload(), 200 );
	}

	/**
	 * Set the emergency presentation-only override and append an audit entry.
	 *
	 * @param WP_REST_Request $request REST request.
	 * @return WP_REST_Response|WP_Error
	 */
	public static function save_admin_appointments_experience( $request ) {
		if ( ! class_exists( 'MMED_Feature_Flags' ) ) {
			return new WP_Error( 'mmed_feature_flags_unavailable', 'Feature flags are unavailable.', array( 'status' => 503 ) );
		}

		$before  = MMED_Feature_Flags::is_enabled( 'appointments_force_classic' );
		$enabled = rest_sanitize_boolean( $request->get_param( 'force_classic' ) );
		$result  = MMED_Feature_Flags::set_flag( 'appointments_force_classic', $enabled );
		...
		if ( $before !== $enabled ) {
			$audit = get_option( self::APPOINTMENTS_FORCE_CLASSIC_AUDIT_OPTION, array() );
			$audit = is_array( $audit ) ? $audit : array();
			array_unshift(
				$audit,
				array(
					'id'            => wp_generate_uuid4(),
					'changed_at_utc' => gmdate( 'c' ),
					'actor_user_id'  => get_current_user_id(),
					'from'           => $before,
					'to'             => $enabled,
				)
			);
			update_option( self::APPOINTMENTS_FORCE_CLASSIC_AUDIT_OPTION, array_slice( $audit, 0, 50 ), false );
		}
	...
	/**
	 * Build a student-safe effective Appointments presentation payload.
	 *
	 * @param int $user_id WordPress user ID.
	 * @return array
	 */
	private static function appointments_experience_payload( $user_id ) {
		$preference = sanitize_key( get_user_meta( absint( $user_id ), self::APPOINTMENTS_EXPERIENCE_META_KEY, true ) );
		if ( ! self::is_valid_appointments_experience( $preference ) ) {
			$preference = 'storyforge';
		}
		$forced = class_exists( 'MMED_Feature_Flags' ) && MMED_Feature_Flags::is_enabled( 'appointments_force_classic' );

		return array(
			'preference'           => $preference,
			'effective_experience' => $forced ? 'classic' : $preference,
			'forced'               => $forced,
		);
	}

	/**
	 * Build the admin-only override payload.
	 *
	 * @return array
	 */
	private static function admin_appointments_experience_payload() {
		$audit = get_option( self::APPOINTMENTS_FORCE_CLASSIC_AUDIT_OPTION, array() );

		return array(
			'force_classic' => class_exists( 'MMED_Feature_Flags' ) && MMED_Feature_Flags::is_enabled( 'appointments_force_classic' ),
			'flag'          => 'appointments_force_classic',
			'audit'         => is_array( $audit ) ? array_slice( $audit, 0, 50 ) : array(),
		);
	}
