(function () {
  ...
  function fetchWidgetToken(mode) {
    ...
    return fetch(String(config.restUrl || "").replace(/\/$/, "") + "/webex/guest-token", {
      method: "GET",
      credentials: "same-origin",
      headers: {
  ...
  function hydrateMeetingFromTeamState() {
    var stateUrl = config.restUrl.replace(/\/$/, "") + "/live-drills-preview/team-challenge";
    var headers = {
      "Accept": "application/json"
    };
    if (config.nonce) {
    ...
    return window.fetch(stateUrl + "?mmed_webex_state_ts=" + Date.now(), {
      method: "GET",
      credentials: "same-origin",
      cache: "no-store",
      headers: headers
      ...
      return response.json().catch(function () {
        ...
        if (!response.ok) {
          throw new Error(payload.message || "Could not load the selected Webex meeting.");
        }
  ...
  function showWidgetFallbackError(stage, message) {
    ...
    status(stage, message || "Embedded Webex could not start. Use the Webex app fallback.", "error");
  }
  ...
  function hasWidgetJoinControl(mount) {
    return !!(mount && mount.querySelector([
      'button[aria-label="Join meeting"]',
      'button[title="Join meeting"]',
      '.wxc-interstitial-meeting button',
      '.wxc-meeting-control button'
    ].join(",")));
  }

  function hasWidgetMeetingMedia(mount) {
    var text = widgetText(mount);
    if (/join meeting/i.test(text) && /my preview/i.test(text)) {
      return false;
    }
  ...
  function runtimeWebexErrorMessage(error) {
    ...
    if (/wdm\/api\/v1\/devices|HTTP Status 403|Forbidden|webex-js-sdk/i.test(message)) {
      return activeRuntimeTokenMode === "host"
        ...
        : "Webex rejected the in-browser device registration. Use the Webex app fallback or refresh and try again.";
    }
    return "";
  }
  ...
  function watchWidgetReadiness(stage, tokenMode) {
    ...
    function check() {
      ...
      if (hasWidgetMeetingMedia(mount)) {
        ...
        status(stage, activeRuntimeTokenMode === "host"
          ...
          : "Embedded Webex video is active inside this Daily Drills stage.", "ready");
        return;
      }
      ...
      if (hasWidgetJoinControl(mount)) {
        status(stage, activeRuntimeTokenMode === "host"
          ...
          : "Webex join screen loaded. Click Join meeting in the Webex panel to enter the room.", "ready");
        return;
      }
      ...
      if (hasWidgetErrorText(mount)) {
        showWidgetFallbackError(stage, activeRuntimeTokenMode === "host"
          ...
          : "Webex could not finish browser join. Use the Webex app fallback or refresh and try again.");
      }
    }
    ...
    widgetReadinessTimer = window.setTimeout(function () {
      ...
      showWidgetFallbackError(stage, activeRuntimeTokenMode === "host"
        ...
        : "Webex did not reach the join screen in this browser. Refresh or use the Webex fallback link.");
    }, 18000);
  ...
  function setupHostControlDrawer(stage) {
    var drawer;
    if (!stage || !isAdminControlMode()) return;
  ...
  function primeCameraPermission(stage) {
    ...
    syncSelectedCameraPatch();
    status(stage, "Checking camera permission before Webex starts...", "loading");
  ...
  function applySelectedCamera(stage, restartWidget) {
    ...
    navigator.mediaDevices.getUserMedia(getVideoConstraints()).then(function (stream) {
      ...
      if (!restartWidget || !stage.classList.contains("is-webex-embedded")) {
        status(stage, "Camera preference saved. Webex controls the final device selection; if it still shows the old camera, choose it inside Webex settings or Chrome site permissions.", "ready");
        return;
      }

      status(stage, "Camera saved. Restarting embedded Webex with that camera...", "loading");
      stopExistingWidget();
      stage.classList.remove("is-webex-embedded");
  ...
  function setupCameraTools(stage) {
    ...
    stage.addEventListener("change", function (event) {
      ...
      status(stage, "Camera choice saved. Use Apply camera to restart embedded Webex with it.", "ready");
    });
  ...
  function installWidgetDeviceHint(stage, mount) {
    ...
    mount.addEventListener("change", function (event) {
      ...
      status(stage, "Webex camera setting changed. If the live camera does not switch, use Apply camera above to restart the embedded room with that device.", "ready");
    }, true);
  }
  ...
  function startEmbedded(stage, options) {
    ...
    if (isAdmin && !options.allowAdmin) {
      status(stage, "Admin mode is not auto-joined. Select or paste the Webex meeting, then use Join in app as host to start the Webex stage inside this browser.", "ready");
      return;
    }

    if (!mount || !destination) {
      if (mount && !destination && !options.hydratedMeeting) {
        status(stage, "Syncing the selected Webex room...", "loading");
        return hydrateMeetingFromTeamState().then(function () {
          startEmbedded(stage, Object.assign({}, options, { hydratedMeeting: true }));
        }).catch(function (error) {
          status(stage, error && error.message ? error.message : "The host has not selected a Webex meeting yet.", "error");
        });
      }
      status(stage, isAdmin ? "Choose or paste the Webex meeting first, then join the in-app host stage." : "The host has not selected a Webex meeting yet. Keep this page open and try again when Dr J starts the room.", "error");
      return;
    }

    if (!window.MmedWebexWidget || typeof window.MmedWebexWidget.init !== "function") {
      status(stage, "Embedded Webex bundle is not loaded. Use the fallback link.", "error");
      return;
    }
    ...
    status(stage, tokenMode === "host"
      ...
      : "Starting embedded Webex inside Daily Drills...", "loading");
    ...
    cameraReady.then(function () {
      ...
      status(stage, "Embedded Webex is loading its join screen...", "loading");
  ...
  function enhanceStage(stage) {
    ...
    var title = meeting.title || "WEBEX-TEST-DO-NOT-USE Legacy Daily Drills Live Preview";
    var isAdmin = isAdminControlMode();
    var introCopy = isAdmin
      ...
      : "Start or join the Webex meeting inside the Daily Drills video stage. The regular Webex app link stays available as a fallback.";
    var primaryAction = isAdmin
      ? '<button class="open-webex-btn" type="button" data-start-embedded-webex data-admin-browser-join>Join in app as host</button>'
      : '<button class="open-webex-btn" type="button" data-start-embedded-webex>Join Webex Inside Daily Drills</button>';
    var initialStatus = isAdmin
      ...
      : "Ready for live Webex join.";

    main.setAttribute("data-webex-enhanced", "1");
    ensureCameraStyles();
	    main.innerHTML = [
      '<div class="live-lockup mmed-webex-embed-intro">',
        '<div class="live-pill"><span class="live-pulse"></span> Live Webex room</div>',
        '<h1 class="live-headline">Daily Drills Live</h1>',
        '<p class="live-copy">' + attr(introCopy) + "</p>",
        '<div class="mmed-webex-embed-actions">',
          primaryAction,
          joinUrl ? '<a class="mmed-webex-fallback-link" href="' + attr(joinUrl) + '" target="_blank" rel="noopener noreferrer">Open Webex app fallback</a>' : "",
        "</div>",
        '<p class="preview-meeting-copy">' + attr(title) + "</p>",
