...
(function () {
	...
	function mediaHTML(a, uid, kind) {
		const locked = lockedArtURL(a, kind === 'detail' ? 'cinematic' : 'pencil-registered');
		if (locked) {
			if (kind === 'detail') {
	...
	function cardHTML(a, uid, admin) {
		...
		const locked = !!lockedArtURL(a, 'pencil-registered');
		return `<div class="mmdv2-card-wrap">
			<button type="button" class="mmdv2-card${locked ? ' mmdv2-card-locked' : ''}" data-open="${a.id}"${locked ? ` data-morph-card="${esc(a.id)}"` : ''} style="--hue:${esc(a.hue || '#ffb340')};--mmdv2-morph-progress:0" aria-label="${esc(a.name)} — ${esc(sub)}">
				...
				${locked ? `<span class="mmdv2-cinematic-copy" aria-hidden="true"><span class="mmdv2-cine-head"><strong class="mmdv2-cine-title">${esc(a.name)}</strong><span class="mmdv2-cine-sub">${esc(sub)}</span></span><span class="mmdv2-cine-support">${esc(a.one || sub)}</span></span>` : ''}
...
void main(){
  ...
  vec4 pencil=texture(u_pencil,v_uv);
  vec4 cinema=texture(u_cinematic,v_uv);
  vec2 px=1.0/u_resolution;
  float l0=lum(pencil.rgb);
  float edge=smoothstep(0.025,0.24,abs(l0-lum(texture(u_pencil,v_uv+vec2(px.x*2.0,0)).rgb))+abs(l0-lum(texture(u_pencil,v_uv+vec2(0,px.y*2.0)).rgb)));
  float field=fbm(v_uv*3.15+vec2(2.4,-1.7));
  float regional=smoothstep(-0.18,0.22,(t-0.50)*1.65+(field-0.5)*0.62);
  float global=smoothstep(0.12,0.92,t);
  float material=clamp(global*0.44+regional*0.56,0.0,1.0);
  float activation=smoothstep(0.025,0.18,t)*(1.0-smoothstep(0.48,0.78,t));
  float graphite=edge*(1.0-smoothstep(0.48,0.90,t));
  vec3 linearP=pow(max(pencil.rgb,vec3(0.0)),vec3(2.2));
  vec3 linearC=pow(max(cinema.rgb,vec3(0.0)),vec3(2.2));
  vec3 color=pow(mix(linearP,linearC,material),vec3(1.0/2.2));
  color=mix(color,pencil.rgb,graphite*0.16);
  color+=vec3(0.26,0.14,0.055)*edge*activation;
  float settle=smoothstep(0.70,0.97,t);
  color=(color-0.5)*(1.0+0.055*settle)+0.5;
  outColor=vec4(clamp(color,0.0,1.0),mix(pencil.a,cinema.a,material));
}`;

		function phaseOf(progress) {
			if (progress <= .001) { return 'pencil'; }
			if (progress < .24) { return 'line-activation'; }
			if (progress < .72) { return 'materialization'; }
			if (progress < .999) { return 'cinematic-settle'; }
			return 'cinematic';
		}
		...
		async function start(node, on) {
			...
			if (active) { releaseContext(active, active.node._mmdv2MorphWanted ? 'cinematic' : 'pencil'); }
