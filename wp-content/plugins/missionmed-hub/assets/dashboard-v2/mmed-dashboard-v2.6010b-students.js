...
(function () {
	...
	const morphEnabled = cfg.experience === 'matrix2';
	const fullMotionPreview = morphEnabled && !!cfg.is_admin && new URLSearchParams(window.location.search).get('mx_dash_6010b_motion') === 'full';
	if (fullMotionPreview) { document.documentElement.classList.add('mmdv2-full-motion-preview'); }
	const reduceMorphMotion = () => !fullMotionPreview && reduceMotion();
	const lockedArtRoot = String(cfg.asset_base || '').replace(/\/?$/, '/') + 'locked-art/';
	const approvedArtSlug = Object.freeze({ homebase: 'homebase', calendar: 'calendar', scheduler: 'scheduler', storyforge: 'storyforge', ivprep: 'iv-prep-on-call', rise: 'rise', ranklist: 'ranklist-iq', lor: 'lor-studio' });
	const lockedArtURL = (a, phase) => {
		if (!(morphEnabled && a && !(a.card_image || a.detail_image) && approvedArtSlug[a.id])) { return ''; }
		const slug = approvedArtSlug[a.id];
		return lockedArtRoot + 'founder-approved/' + phase + '/' + encodeURIComponent(slug + '-' + phase + '-approved.png');
	};
	...
	/* ------------------------------------------------------------------ */
	/* Locked-art WebGL morph — MX-DASH-6010B Founder-approved students.  */
	/* ------------------------------------------------------------------ */
	const morph = (() => {
		const duration = 960;
		const diag = window.MMED_DASH_MORPH_DIAGNOSTICS = {
			canary: false, studentRelease: morphEnabled, mode: 'idle', transitions: 0, frames: 0,
			...
			shaderCompileMs: 0, lastApp: null, endpoint: 'pencil', progress: 0,
			phase: 'pencil', fullMotionPreview, systemReducedMotion: !!reduceMotion(), forced: false
		};
		...
		const profiles = Object.freeze({
			homebase: { shift: [.003704, 0], seed: .11 }, calendar: { shift: [.007407, .01], seed: .23 },
			scheduler: { shift: [.011111, .008333], seed: .37 }, storyforge: { shift: [.007407, .005], seed: .49 },
			ivprep: { shift: [.003704, .03], seed: .61 }, rise: { shift: [.018519, .026667], seed: .73 },
			ranklist: { shift: [.027778, .021667], seed: .84 }, lor: { shift: [.007407, .03], seed: .94 }
		});
...
uniform vec2 u_registration;
uniform vec2 u_cinema_scale;
in vec2 v_uv;
out vec4 outColor;
float hash(vec2 p){return fract(sin(dot(p,vec2(127.1,311.7)))*43758.5453123);}
float noise(vec2 p){vec2 i=floor(p);vec2 f=fract(p);f=f*f*(3.0-2.0*f);return mix(mix(hash(i),hash(i+vec2(1,0)),f.x),mix(hash(i+vec2(0,1)),hash(i+vec2(1,1)),f.x),f.y);}
float fbm(vec2 p){float v=0.0;float a=0.55;for(int i=0;i<4;i++){v+=a*noise(p);p=p*2.03+vec2(17.7,9.2);a*=0.48;}return v;}
float lum(vec3 c){return dot(c,vec3(0.2126,0.7152,0.0722));}
vec3 toLinear(vec3 c){return mix(c/12.92,pow((c+.055)/1.055,vec3(2.4)),step(vec3(.04045),c));}
vec3 toSrgb(vec3 c){return mix(c*12.92,1.055*pow(max(c,vec3(0.0)),vec3(1.0/2.4))-.055,step(vec3(.0031308),c));}
float localReveal(vec2 center,float start,float finish,float grain){
  float spatial=(distance(v_uv,center)-.34)*.10+grain;
  return clamp((u_progress-start-spatial)/(finish-start),0.0,1.0);
}
void main(){
  vec2 cinemaBase=clamp((v_uv-.5)*u_cinema_scale+.5,vec2(0.0),vec2(1.0));
  if(u_progress<=0.00001){outColor=texture(u_pencil,v_uv);return;}
  if(u_progress>=0.99999){outColor=texture(u_cinematic,cinemaBase);return;}
  float p=clamp(u_progress,0.0,1.0);
  float t=p*p*(3.0-2.0*p);
  float align=smoothstep(.03,.30,p)*(1.0-smoothstep(.78,.98,p));
  vec2 pencilUV=clamp(v_uv+u_registration*align*.18,vec2(0.0),vec2(1.0));
  vec2 cinemaUV=clamp(cinemaBase-u_registration*(1.0-t)*.62,vec2(0.0),vec2(1.0));
  vec4 pencil=texture(u_pencil,pencilUV);
  vec4 cinema=texture(u_cinematic,cinemaUV);
  vec2 px=1.0/u_resolution;
  float l0=lum(pencil.rgb);
  float edge=smoothstep(.022,.19,abs(l0-lum(texture(u_pencil,pencilUV+vec2(px.x*2.0,0)).rgb))+abs(l0-lum(texture(u_pencil,pencilUV+vec2(0,px.y*2.0)).rgb)));
  float grain=(fbm(v_uv*4.2+vec2(u_seed*19.0,u_seed*7.0))-.5)*.10;
  float material=(localReveal(vec2(.24+.08*u_seed,.24),.08,.60,grain)+
    localReveal(vec2(.73,.29+.05*u_seed),.14,.66,-grain*.65)+
    localReveal(vec2(.31,.69),.20,.72,grain*.45)+
    localReveal(vec2(.72-.05*u_seed,.76),.25,.78,-grain*.35))*.25;
  material=smoothstep(0.0,1.0,clamp(material,0.0,1.0));
  float saturation=.18+.82*clamp((p-.18)/.58,0.0,1.0);
  vec3 developed=mix(vec3(lum(cinema.rgb)),cinema.rgb,saturation);
  vec3 color=toSrgb(mix(toLinear(pencil.rgb),toLinear(developed),material));
  float activation=smoothstep(.06,.22,p)*(1.0-smoothstep(.46,.72,p));
  float graphite=edge*(1.0-smoothstep(.52,.91,p));
  color=mix(color,pencil.rgb,graphite*.20);
  color+=vec3(.22,.115,.04)*edge*activation;
  float settle=smoothstep(.72,.98,p);
  color=(color-.5)*(1.0+.045*settle)+.5;
  outColor=vec4(clamp(color,0.0,1.0),mix(pencil.a,cinema.a,material));
}`;

		function phaseOf(progress) {
			if (progress <= .001) { return 'pencil'; }
			if (progress < .15) { return 'graphite-stable'; }
			if (progress < .35) { return 'contour-activation'; }
			if (progress < .65) { return 'multi-region-materialization'; }
			if (progress < .90) { return 'detail-settle'; }
			if (progress < .999) { return 'cinematic-polish'; }
			return 'cinematic';
		}
		function syncProgress(node, progress) {
			const value = Math.max(0, Math.min(1, progress));
			const card = node && node.closest('.mmdv2-card');
			if (card) { card.style.setProperty('--mmdv2-morph-progress', value.toFixed(4)); card.classList.toggle('mmdv2-card-target', value >= .999); }
			if (node) { node.setAttribute('data-morph-progress', value.toFixed(3)); }
			diag.progress = value; diag.phase = phaseOf(value);
		}

		function listen(node, type, fn) { node.addEventListener(type, fn); listeners.push(() => node.removeEventListener(type, fn)); }
		function readyImage(img, deferred) {
			if (!img) { return Promise.reject(new Error('Missing approved endpoint')); }
			if (deferred && !img.getAttribute('src')) { img.src = img.getAttribute(deferred) || ''; }
			const loaded = img.complete && img.naturalWidth ? Promise.resolve(img) : new Promise((resolve, reject) => {
				img.addEventListener('load', () => resolve(img), { once: true });
				img.addEventListener('error', () => reject(new Error('Approved endpoint failed to load')), { once: true });
			});
			return loaded.then(async (value) => {
				if (typeof value.decode === 'function') { try { await value.decode(); } catch (error) { if (!(value.complete && value.naturalWidth)) { throw error; } } }
				return value;
			});
		}
		function loadTarget(node) { return Promise.all([readyImage($('.mmdv2-morph-pencil', node)), readyImage($('.mmdv2-morph-cinematic', node), 'data-src')]); }
		...
		function fallback(node, on, reason) {
			...
			loadTarget(node).then(() => { if (node._mmdv2MorphWanted === on) { node.classList.remove('mmdv2-morph-failed'); node.classList.toggle('is-target', on); } }).catch(() => { diag.failures += 1; node.classList.add('mmdv2-morph-failed'); node.classList.remove('is-target'); });
			diag.mode = reduceMorphMotion() ? 'reduced-motion' : 'css-fallback';
			diag.lastApp = node.getAttribute('data-morph-app'); diag.endpoint = on ? 'cinematic' : 'pencil';
			syncProgress(node, on ? 1 : 0);
			if (reason) { node.setAttribute('data-morph-fallback', reason); }
		}
		function releaseContext(item, endpoint, immediate) {
			if (!item || item.released) { return; }
			if (item.raf) { cancelAnimationFrame(item.raf); item.raf = 0; }
			if (item.releaseFrame) { cancelAnimationFrame(item.releaseFrame); item.releaseFrame = 0; }
			...
			syncProgress(item.node, endpoint === 'cinematic' ? 1 : endpoint === 'pencil' ? 0 : item.progress);
			const finalize = () => {
			...
			if (immediate) { finalize(); } else { item.releaseFrame = requestAnimationFrame(finalize); }
		}
		...
		function render(item) {
			...
			syncProgress(item.node, item.progress);
		}
		function tick(time) {
			const item = active; if (!item) { return; }
			if (document.hidden) { item.raf = 0; item.lastTime = 0; return; }
			if (item.lastTime) {
				const rawInterval = time - item.lastTime, step = Math.min(48, rawInterval);
				const perceptualRate = .55 + .90 * Math.abs(item.progress * 2 - 1);
				item.progress = Math.max(0, Math.min(1, item.progress + item.direction * step / duration * perceptualRate));
		...
		async function start(node, on) {
			...
			if (reduceMorphMotion()) { fallback(node, on, 'reduced-motion'); return; }
			...
			if (active && active.node === node) {
				if (active.releaseFrame) { cancelAnimationFrame(active.releaseFrame); active.releaseFrame = 0; active.node.classList.add('is-running'); }
				active.direction = on ? 1 : -1; active.lastTime = 0; diag.transitions += 1;
				if (!active.raf) { active.raf = requestAnimationFrame(tick); } return;
			}
			if (active) { releaseContext(active, active.node._mmdv2MorphWanted ? 'cinematic' : 'pencil', true); }
			...
			try {
				...
				const profile = profiles[node.getAttribute('data-morph-app')] || { shift: [0, 0], seed: .5 };
				const targetAspect = 595 / 661, sourceAspect = cinematic.naturalWidth / cinematic.naturalHeight;
				const cinemaScale = sourceAspect > targetAspect ? [targetAspect / sourceAspect, 1] : [1, sourceAspect / targetAspect];
				gl.uniform2f(gl.getUniformLocation(program, 'u_registration'), profile.shift[0], profile.shift[1]);
				gl.uniform2f(gl.getUniformLocation(program, 'u_cinema_scale'), cinemaScale[0], cinemaScale[1]);
				gl.uniform1f(gl.getUniformLocation(program, 'u_seed'), profile.seed);
				...
				diag.forced = false; render(active); node.classList.remove('mmdv2-morph-fallback'); node.classList.add('is-running'); active.raf = requestAnimationFrame(tick);
			} catch (error) {
		...
		function bind(root) {
			...
			if (!nodes.length) { diag.mode = morphEnabled ? 'idle' : 'disabled'; return; }
		...
		function dispose() {
			...
			if (active) { releaseContext(active, active.progress >= .5 ? 'cinematic' : 'pencil', true); }
			destroySharedContext();
		}
		async function seek(appId, progress) {
			if (!(morphEnabled && cfg.morph_debug === true)) { throw new Error('Morph debug is unavailable'); }
			const node = nodes.find((candidate) => candidate.getAttribute('data-morph-app') === appId);
			if (!node) { throw new Error('Unknown morph card'); }
			await start(node, Number(progress) >= .5);
			if (!active || active.node !== node) { throw new Error('Morph renderer unavailable'); }
			if (active.raf) { cancelAnimationFrame(active.raf); active.raf = 0; }
			active.progress = Math.max(0, Math.min(1, Number(progress) || 0)); active.direction = 0; active.lastTime = 0;
			diag.forced = true; render(active);
			return { appId, progress: active.progress, phase: diag.phase, contexts: diag.activeContexts };
		}
		...
		if (morphEnabled && cfg.morph_debug === true) { window.MMED_DASH_MORPH_TEST = { seek, dispose }; }
		return { bind, dispose };
	})();
