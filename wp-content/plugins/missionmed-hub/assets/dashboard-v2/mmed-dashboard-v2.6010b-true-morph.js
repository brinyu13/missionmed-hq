...
(function () {
	...
	const fullMotionPreview = morphCanary && new URLSearchParams(window.location.search).get('mx_dash_6010b_motion') === 'full';
	...
	const lockedArtURL = (a, phase) => {
		if (!(morphCanary && a && !(a.card_image || a.detail_image) && approvedArtSlug[a.id])) { return ''; }
...
void main(){
  ...
  outColor=vec4(clamp(color,0.0,1.0),mix(pencil.a,cinema.a,material));
		...
		async function seek(appId, progress) {
			if (!(morphCanary && cfg.morph_debug === true)) { throw new Error('Morph debug is unavailable'); }
		...
		if (morphCanary && cfg.morph_debug === true) { window.MMED_DASH_MORPH_TEST = { seek, dispose }; }
		return { bind, dispose };
	})();
