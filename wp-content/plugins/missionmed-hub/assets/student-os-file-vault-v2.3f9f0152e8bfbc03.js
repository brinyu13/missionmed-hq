(function (window, document) {
	...
	var MIME_ALIASES = {
		...
		pages: ["application/vnd.apple.pages", "application/x-iwork-pages-sffpages", "application/zip", "application/x-zip-compressed", "application/octet-stream"],
		numbers: ["application/vnd.apple.numbers", "application/x-iwork-numbers-sffnumbers", "application/zip", "application/x-zip-compressed", "application/octet-stream"],
		keynote: ["application/vnd.apple.keynote", "application/x-iwork-keynote-sffkey", "application/zip", "application/x-zip-compressed", "application/octet-stream"],
	...
	FileVaultV2.prototype.handleChange = function (event) {
		...
		if (target.matches("[data-fv2-upload-file]")) {
			...
			if (this.state.upload) {
				...
				this.state.upload.sha256 = "";
				if (!this.state.upload.outputFilenameTouched) this.state.upload.outputFilename = "";
		...
		if (target.matches("[data-fv2-upload-name]") && this.state.upload) {
			...
			this.refreshUploadFilenameField();
		...
		if (target.matches("[data-fv2-upload-type]") && this.state.upload) {
			...
			this.state.upload.versionLabel = "Version01";
			if (!this.state.upload.outputFilenameTouched) this.state.upload.outputFilename = "";
	...
	FileVaultV2.prototype.handleInput = function (event) {
		...
		if (target.matches("[data-fv2-output-filename]") && this.state.upload) {
			this.state.upload.outputFilename = target.value;
			this.state.upload.outputFilenameTouched = true;
			this.updateUploadNextButton();
			return;
		}
		if (target.matches("[data-fv2-upload-version-label]") && this.state.upload) {
			this.state.upload.versionLabel = target.value;
			this.refreshUploadFilenameField();
			this.updateUploadNextButton();
			return;
		}
	...
	FileVaultV2.prototype.openUpload = function (options) {
		...
		this.state.upload = {
			...
			versionLabel: documentItem && documentItem.version_label ? String(documentItem.version_label) : "Version" + String(initialVersion).padStart(2, "0"),
			isFinal: false,
	...
	FileVaultV2.prototype.uploadDetailsMarkup = function () {
		...
		var fileTypeLabel = upload.documentType === "application_photo" ? "JPEG application photo" : "Documents, iWork, OpenDocument, text, images, MP4, or WebM";
		...
		var versionField = '<div class="fv2-field fv2-version-assignment" data-fv2-upload-version aria-label="Internal revision assigned automatically"><span>Internal revision</span><strong>Version ' + esc(Math.max(1, positiveInt(upload.version))) + '</strong><small>Assigned automatically and cannot be changed.</small></div><label class="fv2-field"><span>Visible version label</span><input type="text" maxlength="80" required value="' + escAttr(upload.versionLabel) + '" data-fv2-upload-version-label data-fv2-overlay-focus="upload-version-label" placeholder="Version label"></label>';
	...
	FileVaultV2.prototype.uploadReviewMarkup = function () {
		...
		var versionLabel = "Revision " + esc(upload.version) + " · " + esc(upload.versionLabel || upload.draftLabel) + (upload.isFinal ? " · Final" : "");
		...
		return '<section class="fv2-upload-review"><span>Ready when you are</span><h2>' + esc(upload.displayName || "Document") + '</h2><dl><div><dt>Original file</dt><dd>' + esc(file ? file.name : "No file") + "</dd></div><div><dt>Size</dt><dd>" + esc(file ? formatSize(file.size) : "0 B") + "</dd></div><div><dt>Type</dt><dd>" + esc(this.documentTypeLabel(upload.documentType)) + "</dd></div><div><dt>Course / session</dt><dd>" + esc(upload.program + " / " + upload.sessionLetter) + "</dd></div><div><dt>Next version</dt><dd>" + versionLabel + "</dd></div><div><dt>After upload</dt><dd>" + esc(afterUpload) + "</dd></div></dl><div class=\"fv2-review-note fv2-review-filename\"><span>Saved filename</span><p>" + esc(upload.canonicalName || this.canonicalFilenamePreview()) + "</p></div>" + (upload.note ? '<div class="fv2-review-note"><span>' + (publishesShare ? "Recipient description" : "Note to advisor") + '</span><p>' + esc(upload.note).replace(/\n/g, "<br>") + "</p></div>" : "") + '<p class="fv2-security-copy">' + icon("lock") + (publishesShare ? " The source remains private in R2. Access is granted by current server-owned audience rules." : " Your file stays private. MissionMed verifies it before adding it to Your Files.") + '</p></section><div class="fv2-modal-actions"><button type="button" class="fv2-button fv2-button-secondary" data-fv2-action="upload-back">' + icon("arrowLeft") + 'Back</button><button type="button" class="fv2-button fv2-button-primary" data-fv2-action="upload-start">' + icon("upload") + (publishesShare ? "Upload and share" : "Upload securely") + "</button></div>";
	};
	...
	FileVaultV2.prototype.uploadProgressMarkup = function () {
		...
		if (upload.phase === "success" && upload.result) {
			...
			var resultVersionLabel = String(result.version_label || upload.versionLabel || result.draft_label || "Version" + String(resultVersion).padStart(2, "0"));
			var publishesShare = this.uploadPublishesShare(upload);
			...
			return '<section class="fv2-upload-celebration" role="status"><span class="fv2-celebration-mark">' + icon("check") + '</span><span class="fv2-celebration-kicker">' + (publishesShare ? "Private share confirmed" : "Private upload confirmed") + '</span><h2>' + (publishesShare ? "Your file is verified and shared." : "Your document is safely in the Vault.") + '</h2><p>' + esc(result.name || upload.displayName || "Document") + ' is now ' + esc(resultVersionLabel) + ' (internal revision ' + esc(resultVersion) + ')' + (result.is_final ? " and marked Final" : "") + '.</p><div class="fv2-celebration-facts"><span>' + icon("lock") + '<strong>Private</strong><small>Only the approved audience can open it</small></span><span>' + icon("check") + '<strong>Verified</strong><small>Stored and confirmed by MissionMed</small></span><span>' + icon("journey") + '<strong>' + esc(resultVersionLabel) + '</strong><small>Internal revision ' + esc(resultVersion) + '; earlier revisions remain available</small></span></div><div class="fv2-celebration-name"><span>Saved as</span><strong>' + esc(result.filename || upload.canonicalName || result.name || "Document") + '</strong></div></section><div class="fv2-modal-actions fv2-celebration-actions">' + successActions + '</div>';
		}
		var phaseCopy = {
	...
	FileVaultV2.prototype.applyUploadReplacement = function (replacement) {
		...
		upload.versionLabel = upload.draftLabel;
		if (!upload.outputFilenameTouched) upload.outputFilename = "";
	...
	FileVaultV2.prototype.generatedFilenamePreview = function () {
		var upload = this.state.upload || {};
		var context = this.uploadContext();
		...
		return [
			...
			this.canonicalFilenameToken(upload.versionLabel || upload.draftLabel || ("Version" + String(Math.max(1, positiveInt(upload.version))).padStart(2, "0")), "Version01"),
	...
	FileVaultV2.prototype.canonicalFilenamePreview = function () {
		var upload = this.state.upload || {};
		return String(upload.outputFilename || this.generatedFilenamePreview());
	};
	...
	FileVaultV2.prototype.uploadContract = function (documentType) {
		...
		var fallback = documentType === "application_photo"
			? { extensions: ["jpg", "jpeg"], allow_other_extensions: false, blocked_extensions: [], max_file_size: 153600 }
			: { extensions: COMMON_EXTENSIONS, allow_other_extensions: true, blocked_extensions: BLOCKED_EXTENSIONS, max_file_size: Number(storage.max_file_size) || Number(this.config.maxFileSize) || 26214400 };
		var source = documentType ? (contracts[documentType] || contracts.default || fallback) : fallback;
		var extensions = Array.isArray(source.extensions) ? source.extensions.map(function (extension) { return String(extension).toLowerCase(); }).filter(function (extension) { return /^[a-z0-9]{1,12}$/.test(extension); }) : fallback.extensions;
		var blockedExtensions = Array.isArray(source.blocked_extensions) ? source.blocked_extensions.map(function (extension) { return String(extension).toLowerCase(); }) : fallback.blocked_extensions;
		return {
			extensions: extensions.length ? extensions : fallback.extensions,
			allowOtherExtensions: !!source.allow_other_extensions,
	...
	FileVaultV2.prototype.uploadAccept = function (contract) {
		if (contract && contract.allowOtherExtensions) return "";
		var values = [];
	...
	FileVaultV2.prototype.validateFile = function (file, documentType) {
		...
		var stem = extension && rawName.indexOf(".") !== -1 ? rawName.slice(0, -(extension.length + 1)) : "";
		var contract = this.uploadContract(documentType);
		if (rawName.length > 180 || /[\u0000-\u001f\u007f\\/]/.test(rawName) || /[\u202a-\u202e\u2066-\u2069]/i.test(rawName) || !stem || /^\./.test(stem) || /[.\s]$/.test(stem) || stem.indexOf("..") !== -1 || !/^[a-z0-9]{1,12}$/.test(extension)) return "Choose a safe filename with one final extension.";
		if ((contract.blockedExtensions || BLOCKED_EXTENSIONS).indexOf(extension) !== -1) return "This active or executable file type cannot be stored in File Vault.";
		if (!contract.allowOtherExtensions && contract.extensions.indexOf(extension) === -1) return documentType === "application_photo" ? "Choose a JPEG application photo." : "Unsupported file type.";
		var mime = this.mimeForFile(file);
		if (BLOCKED_MIME_TYPES.indexOf(mime.toLowerCase()) !== -1) return "This active or executable file type cannot be stored in File Vault.";
		var allowedMimes = MIME_ALIASES[extension] || (MIME_FALLBACKS[extension] ? [MIME_FALLBACKS[extension]] : []);
		if (allowedMimes.length && allowedMimes.indexOf(mime) === -1) return "The file MIME type does not match its extension.";
		if (!file.size || file.size > contract.maxFileSize) return "File must be between 1 byte and " + formatSize(contract.maxFileSize) + ".";
		return "";
	};

	FileVaultV2.prototype.validateOutputFilename = function (value, file) {
		value = String(value || "").trim();
		var sourceExtension = String(file && file.name || "").split(".").pop().toLowerCase();
		var extension = value.split(".").pop().toLowerCase();
		var stem = extension && value.indexOf(".") !== -1 ? value.slice(0, -(extension.length + 1)) : "";
		if (value.length > 180 || /[\u0000-\u001f\u007f\\/]/.test(value) || /[\u202a-\u202e\u2066-\u2069]/i.test(value) || !stem || /^\./.test(stem) || /[.\s]$/.test(stem) || stem.indexOf("..") !== -1 || !/^[a-z0-9]{1,12}$/.test(extension)) return "Choose a safe saved filename of 180 characters or fewer.";
		if (sourceExtension && extension !== sourceExtension) return "The saved filename must keep the selected file extension.";
		return "";
	};

	FileVaultV2.prototype.validateVersionLabel = function (value) {
		value = String(value || "").trim();
		if (!value || value.length > 80 || /[\u0000-\u001f\u007f]/.test(value) || /[\u202a-\u202e\u2066-\u2069]/i.test(value)) return "Enter a safe visible version label of 80 characters or fewer.";
		return "";
	};
	...
	FileVaultV2.prototype.uploadStepOneError = function () {
		...
		return this.validateFile(upload.file, upload.documentType) || this.validateOutputFilename(this.canonicalFilenamePreview(), upload.file) || this.validateVersionLabel(upload.versionLabel) || (!upload.documentType ? "Choose a document type." : "") || (!upload.displayName.trim() ? "Enter a document name." : "") || (PROGRAM_OPTIONS.indexOf(upload.program) === -1 ? "Choose a MissionMed course or program." : "") || (SESSION_OPTIONS.indexOf(upload.sessionLetter) === -1 ? "Choose session A through G." : "") || audienceError;
	};
	...
	FileVaultV2.prototype.refreshUploadFilenameField = function () {
		var upload = this.state.upload;
		var field = this.refs.overlay && this.refs.overlay.querySelector("[data-fv2-output-filename]");
		if (!upload || !field || upload.outputFilenameTouched) return;
		field.value = this.generatedFilenamePreview();
	};
	...
	FileVaultV2.prototype.startUpload = function (startAt) {
		...
		function sign() {
			...
			return self.request("POST", path, payload, null, upload.controller && upload.controller.signal).then(function (intent) {
				...
				upload.draftLabel = "Version" + String(upload.version).padStart(2, "0");
				upload.versionLabel = String(intent.version_label || upload.versionLabel || upload.draftLabel);
				upload.canonicalName = String(intent.canonical_name || upload.canonicalName || self.canonicalFilenamePreview());
				upload.outputFilename = upload.canonicalName;
				return put();
			});
		}
