(function () {
  ...
  var lastWinnerKey = "";
  var realtimePollInterval = Math.max(220, Math.min(Number(config.pollInterval || 320), 420));
  var realtimeTurboInterval = 160;
  var realtimeHiddenInterval = 1000;
  ...
  startRealtimePolling();
    ...
    return baseDelay + Math.floor(Math.random() * 36);
	  }
  ...
  function refreshState(options) {
    ...
    return requestState("GET").then(function (nextState) {
	      ...
	      var fingerprint = stateFingerprint(nextState);
		      var previousState = state ? cloneState(state) : null;
	      ...
	      updateMeetingConfigFromState(nextState);
		      var feedback = detectIncomingFeedback(previousState, state);
	      renderState();
    ...
    if (optOutButton) {
      ...
      optOutButton.textContent = "Updating...";
	    ...
	    window.setTimeout(function () {
	      var source = choice === "watch" ? "student_watch_choice" : "student_play_choice";
		  ...
		  function enterStudentWebexFromPanel(button) {
		    ...
		    if (button) {
		      button.disabled = true;
		      button.innerHTML = 'Opening Webex...<span>Check the video stage above</span>';
		      window.setTimeout(function () {
		        if (!button || !button.isConnected) {
		          return;
		        }
		        button.disabled = false;
		        button.innerHTML = 'Enter Webex Meeting<span>Join the host in the live video room</span>';
		      }, 5000);
		    }
	  ...
	  function startCountdownPreset(button) {
	    ...
	    setCountdown("start", seconds);
		  ...
		  function viewerStudentEntry() {
		    ...
		    var name = currentUserId ? "You" : [profile.firstName || "", profile.lastName || ""].join(" ").trim();
		    name = name || "You";
		    return {
