 ...
 * Version: 0.1.0
 * Author: MissionMed (Fable 5.1 prototype)
 * Requires PHP: 7.4
 */
...
add_action(
	...
	function () {
		...
		try {
			foreach ( array( 'store', 'docx', 'region', 'root-source', 'rise-client', 'evidence-bundle', 'tiers', 'provider', 'generator', 'rest', 'page' ) as $part ) {
				mmps_require( $part );
			}
			MMPS_Rest::init();
			MMPS_Page::init();
