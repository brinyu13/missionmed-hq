...
export const SCHEDULER_ROUTE_FAMILIES = {
  student: [
    ...
    'GET /api/scheduler/appointments/:appointmentId/recording',
...
export async function handleSchedulerApiRoute(request, response, url, context = {}) {
  ...
  try {
    ...
    const appointmentRecordingMatch = pathname.match(/^\/api\/scheduler\/appointments\/([^/]+)\/recording$/u);
    if (appointmentRecordingMatch) {
      if (request.method !== 'GET') return methodNotAllowed(sendMethodNotAllowed, response, ['GET']);
      const appointmentId = decodeURIComponent(appointmentRecordingMatch[1]);
      const result = await getSchedulerAppointmentRecording({
        appointmentId,
        actor,
        repository,
        schedulerAdapters,
      });
      const statusCode = result.ok || ['processing', 'not_available'].includes(result.status) ? 200 : result.status_code || 404;
      sendJson(
        response,
        statusCode,
        result.ok ? schedulerOk(result.data) : schedulerError(result.error, result.message, result.data || {}),
        authHeaders,
      );
      return true;
    }
...
async function createAndPersistMeetingLink({
  ...
  const studentDisplayName = actor.displayName || actor.login || null;
  ...
  const meeting = await adapter({
    ...
    webex_auto_record: config.provider === 'webex' ? config.auto_record : false,
    actor,
  });
  ...
  const meetingPatch = {
    ...
    auto_record: config.provider === 'webex' ? config.auto_record === true : false,
...
function schedulerMeetingTitle({ providerName = '', appointmentTypeName = '', provider = '', studentDisplayName = '', payload = {} } = {}) {
  const explicit = safeTextValue(payload.meeting_title || payload.meetingTitle || '');
  const mentor = safeTextValue(providerName) || 'MissionMed provider';
  const type = safeTextValue(
    appointmentTypeName
      || payload.appointment_type_name
      || payload.appointmentTypeName
      || '',
  ) || 'Scheduler appointment';
  const student = safeTextValue(
    studentDisplayName
  ...
  if (String(provider || '').toLowerCase() === 'webex' && student) {
    return `${student} & ${mentor} - ${type}`;
  }
  if (explicit) return explicit;
  const platformLabel = 'MissionMed Appointment';
...
function webMeetingConfig(appointmentType = {}, providerRow = null) {
  ...
  return {
    ...
    auto_record: configBooleanDefault(
      scopedMeeting.auto_record
        ?? scopedMeeting.autoRecord
        ?? scopedMeeting.recording_auto_record
        ?? scopedMeeting.recordingAutoRecord,
      provider === 'webex',
    ),
...
async function getSchedulerAppointmentRecording({
  appointmentId = '',
  actor,
  repository,
  schedulerAdapters = {},
} = {}) {
  const appointment = typeof repository.getAppointmentById === 'function'
    ? await repository.getAppointmentById(appointmentId)
    : null;
  if (!appointment) {
    return {
      ok: false,
      status: 'not_found',
      status_code: 404,
      error: 'scheduler_appointment_not_found',
      message: 'Appointment was not found.',
    };
  }

  if (!canAccessAppointmentRecording(actor, appointment)) {
    return {
      ok: false,
      status: 'forbidden',
      status_code: 403,
      error: 'scheduler_recording_forbidden',
      message: 'Recording is not available for this user.',
    };
  }

  const metadata = normalizeCalendarMetadata(appointment.metadata);
  const integrations = normalizeCalendarMetadata(metadata.scheduler_integrations);
  const meeting = normalizeCalendarMetadata(integrations.meeting);
  const provider = appointmentMeetingProvider(appointment) || meeting.meeting_provider || meeting.provider;
  if (String(provider || '').toLowerCase() !== 'webex') {
    return {
      ok: false,
      status: 'not_available',
      error: 'scheduler_recording_not_webex',
      message: 'Recording lookup is only available for Webex appointments.',
      data: {
        status: 'not_available',
        has_recording: false,
  ...
  const existingRecording = normalizeCalendarMetadata(integrations.recording);
  if (existingRecording.status === 'ready' && existingRecording.playback_url) {
    return {
      ok: true,
      status: 'ready',
      data: schedulerRecordingResponse({
        appointment,
        meeting,
  ...
  if (!webexMeetingId) {
    return {
      ...
      message: 'Webex meeting id is not available for this appointment yet.',
      data: {
        status: 'processing',
        has_recording: false,
        appointment_id: String(appointment.id || appointmentId),
      },
    };
  }

  const appointmentRecordingTitle = await resolveAppointmentRecordingTitle({
    appointment,
    meeting,
    metadata,
    repository,
  });
  const lookupAdapter = schedulerAdapters.webexRecordingLookupAdapter || webexRecordingLookupAdapter;
  const lookup = await lookupAdapter({
    appointment_id: appointment.id || appointmentId,
    meeting_id: webexMeetingId,
    webex_meeting_id: webexMeetingId,
    appointment_title: appointmentRecordingTitle,
    appointment_start_at: appointment.start_at || appointment.startAt || '',
    appointment_end_at: appointment.end_at || appointment.endAt || '',
  });
  if (!lookup.ok || !lookup.recording?.playback_url) {
    return {
      ok: false,
      status: lookup.status === 'not_configured' ? 'not_configured' : 'processing',
      error: 'scheduler_recording_processing',
      message: lookup.message || 'Recording is still processing.',
      data: {
        status: lookup.status || 'processing',
        has_recording: false,
        appointment_id: String(appointment.id || appointmentId),
  ...
  const recording = sanitizeSchedulerRecordingSnapshot(lookup.recording);
  const nextMetadata = {
    ...metadata,
    scheduler_integrations: {
      ...integrations,
  ...
  if (typeof repository.updateAppointmentIntegration === 'function') {
    await repository.updateAppointmentIntegration(appointment.id || appointmentId, { metadata: nextMetadata });
    if (resultAppointmentMutable(appointment)) {
      appointment.metadata = nextMetadata;
    }
  }
  ...
  return {
    ...
    data: schedulerRecordingResponse({
      appointment,
      meeting,
      recording: nextMetadata.scheduler_integrations.recording,
      source: 'webex_lookup',
    }),
  };
}

function canAccessAppointmentRecording(actor = {}, appointment = {}) {
  if (actor.isAdmin) return true;
  if (actor.isProvider) {
    const providerId = String(actor.schedulerProviderId || actor.providerId || '').trim();
    return Boolean(providerId && providerId === String(appointment.provider_id || appointment.providerId || ''));
  }
  return String(actor.userId || '') === String(appointment.student_user_id || appointment.studentUserId || '');
}

function sanitizeSchedulerRecordingSnapshot(recording = {}) {
  return {
    id: String(recording.id || '').trim(),
    title: String(recording.title || '').trim(),
    playback_url: String(recording.playback_url || '').trim(),
    download_url_present: recording.download_url_present === true,
    duration_seconds: Number(recording.duration_seconds || 0) || 0,
    file_size_bytes: Number(recording.file_size_bytes || 0) || 0,
    created_at: String(recording.created_at || '').trim(),
  };
}

async function resolveAppointmentRecordingTitle({
  appointment = {},
  meeting = {},
  metadata = {},
  repository,
} = {}) {
  const explicitTitle = safeTextValue(
    meeting.title
  ...
  const integrations = normalizeCalendarMetadata(metadata.scheduler_integrations);
  const bookingStudent = normalizeCalendarMetadata(integrations.booking_student);
  const studentName = safeTextValue(bookingStudent.display_name || bookingStudent.displayName || '');
  const [providerName, appointmentTypeName] = await Promise.all([
    resolveSchedulerProviderDisplayName(repository, appointment.provider_id || appointment.providerId),
    resolveSchedulerAppointmentTypeName(repository, appointment.appointment_type_id || appointment.appointmentTypeId),
  ]);

  if (studentName && providerName && appointmentTypeName) {
    return `${studentName} & ${providerName} - ${appointmentTypeName}`;
  }
  if (providerName && appointmentTypeName) {
    return `MissionMed Appointment - ${providerName} - ${appointmentTypeName}`;
  }
  return safeTextValue(appointment.appointment_type_name || appointment.appointmentTypeName || appointmentTypeName || '');
}

async function resolveSchedulerProviderDisplayName(repository, providerId = '') {
  if (!providerId || typeof repository?.listProviders !== 'function') return '';
  try {
    const rows = await repository.listProviders({ providerId });
    const provider = Array.isArray(rows) ? rows.find((row) => String(row.id || '') === String(providerId)) || rows[0] : null;
    return safeTextValue(provider?.display_name || provider?.displayName || provider?.name || '');
  } catch {
    return '';
  }
}

async function resolveSchedulerAppointmentTypeName(repository, appointmentTypeId = '') {
  if (!appointmentTypeId || typeof repository?.listAppointmentTypes !== 'function') return '';
  try {
    const rows = await repository.listAppointmentTypes({ appointmentTypeId });
    const type = Array.isArray(rows) ? rows.find((row) => String(row.id || '') === String(appointmentTypeId)) || rows[0] : null;
    return safeTextValue(type?.name || type?.display_name || type?.displayName || '');
  } catch {
    return '';
  }
}
...
function schedulerRecordingResponse({
  appointment = {},
  meeting = {},
  recording = {},
  source = 'unknown',
  ...
  return {
    status: 'ready',
    has_recording: true,
    appointment_id: String(appointment.id || appointment.appointment_id || appointment.appointmentId || ''),
    meeting_provider: 'webex',
    webex_meeting_id: String(meeting.webex_meeting_id || meeting.external_event_id || appointment.external_event_id || ''),
    recording: sanitizeSchedulerRecordingSnapshot(recording),
    recording_url: String(recording.playback_url || ''),
    playback_url: String(recording.playback_url || ''),
...
function buildSchedulerCalendarFeedEvent(appointment = {}, { provider = {}, appointmentType = {}, audience = 'student' } = {}) {
  ...
  const recording = normalizeCalendarMetadata(metadata.scheduler_integrations?.recording);
  const meetingUrl = appointmentMeetingUrl(appointment);
  const meetingProvider = appointmentMeetingProvider(appointment, appointmentType);
  const joinLabel = meetingUrl ? joinButtonLabel(meetingProvider) : null;
  const recordingReady = String(recording.status || '').toLowerCase() === 'ready' && Boolean(recording.playback_url);
  const recordingExpected = meetingProvider === 'webex' && (meeting.auto_record === true || recordingReady);
  ...
  return {
    ...
    has_recording: recordingReady,
    recording_status: recordingReady ? 'ready' : recordingExpected ? 'processing' : 'not_expected',
    recording_url: recordingReady ? recording.playback_url : null,
    ...
    meta_json: {
      ...
      webex_meeting_id: meeting.webex_meeting_id || null,
      recording_status: recordingReady ? 'ready' : recordingExpected ? 'processing' : 'not_expected',
      has_recording: recordingReady,
      recording_url: recordingReady ? recording.playback_url : null,
