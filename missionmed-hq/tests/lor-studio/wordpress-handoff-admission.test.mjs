...
function harness({ audience = '', issue = 'success' } = {}) {
  const callback = `https://missionmed-hq-production.up.railway.app/api/lor-studio/auth/callback?audience=lor-studio&state=${'a'.repeat(64)}`;
  return `
...
test('source isolates the exact LOR branch before the unchanged generic signed-identity path', async () => {
  const source = await readFile(handoffPath, 'utf8');
  const lorBranch = source.indexOf("if ('lor-studio' === $audience_raw)");
  const genericPayload = source.indexOf('$payload = mmhq_handoff_build_token_payload($wp_user);');
