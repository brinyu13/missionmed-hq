...
export const DR133_RUNTIME_CA_TRANSFER_CONTRACT =
  'missionmed.lor.dr133-production-runtime-ca-transfer.v1';
...
const ROOT_PREFIX = 'f2-lor-dr133-production-ca-';
const ROOT_NAME = /^f2-lor-dr133-production-ca-[A-Za-z0-9_-]{6,}$/u;
...
function signalProcessGroup(pid, signal) {
  if (!Number.isSafeInteger(pid) || pid < 1 || signal !== 'SIGKILL') return false;
  try {
...
export function createSecretSafeRailwayCommandRunner(executablePath, options = {}) {
  ...
  if (!Number.isSafeInteger(processGroupProbeDelayMs)
  ...
  return async (descriptor) => {
    ...
    const outcome = await new Promise((resolve) => {
      ...
      const leaderClose = new Promise((resolveClose) => {
        resolveLeaderClose = resolveClose;
      });

      const settle = (uncertainChild) => {
        if (settled) return;
        settled = true;
      ...
      const groupAlive = async () => {
        try {
          const alive = await probeProcessGroup(child?.pid);
          return alive === true ? true : alive === false ? false : null;
        } catch {
          return null;
        }
      };

      const proveGroupEmpty = async (waitMs) => {
        const attempts = Math.max(1, Math.ceil(waitMs / processGroupProbeDelayMs));
        for (let attempt = 0; attempt < attempts; attempt += 1) {
          const alive = await groupAlive();
          if (alive === false) return true;
          if (alive !== true) return false;
          if (attempt + 1 < attempts) await sleepFn(processGroupProbeDelayMs);
        }
        return false;
      };

      const waitForLeaderClose = async () => {
        if (closeObserved) return true;
        await Promise.race([leaderClose, sleepFn(killGraceMs)]);
        return closeObserved;
      };

      const terminateAndProveGroupEmpty = async () => {
        const alive = await groupAlive();
        if (alive === false) return true;
        if (alive !== true) return false;
        let signaled = false;
        try {
          signaled = await signalGroup(child.pid, 'SIGKILL') === true;
        } catch {
          signaled = false;
        }
        if (!signaled) return false;
        return await proveGroupEmpty(killGraceMs);
      };

      const finish = async ({ terminateRequested = false } = {}) => {
        ...
        if (childStarted) {
          if (terminateRequested) {
            if (!await terminateAndProveGroupEmpty()) killFailed = true;
            await waitForLeaderClose();
          } else {
            const emptyAtLeaderClose = await proveGroupEmpty(0);
            if (!emptyAtLeaderClose) {
              unexpectedDescendant = true;
              if (!await terminateAndProveGroupEmpty()) killFailed = true;
            }
          }
        }
        const finalGroupEmpty = !childStarted || await proveGroupEmpty(0);
        settle(childStarted && (
          !closeObserved || !finalGroupEmpty || unexpectedDescendant
        ));
      };

      const terminate = () => {
        if (settled || finalizing) return;
        void finish({ terminateRequested: true });
      };
      try {
...
export async function createDr133DedicatedRailwaySshAgentVerifier(rawOptions = {}) {
  if (!plain(rawOptions)
    || Reflect.ownKeys(rawOptions).some(
      (key) => typeof key !== 'string' || !['cwd', 'environment'].includes(key),
    )) fail('OPTIONS_INVALID');
  const environment = rawOptions.environment ?? process.env;
  const cwd = rawOptions.cwd ?? process.cwd();
  safeAbsolutePath(cwd, 'COMMAND_CWD_INVALID');
  const anchor = await anchorSshAgentSocket(environment?.SSH_AUTH_SOCK);
  await verifyDedicatedRailwaySshAgent(anchor, cwd);
  return Object.freeze({
    socketPath: anchor.path,
    verify: async () => {
      await verifyDedicatedRailwaySshAgent(anchor, cwd);
      return true;
    },
...
function validateDownloadReceipt(bytes, localPath) {
  ...
  if (
    ...
    || value.service.name !== 'Postgres-3TCU'
...
export function assertDr133RuntimeTargetRootCa(validated) {
  if (!plain(validated)
    || validated.sha256 !== DR133_PRODUCTION_DATABASE_CA_DER_SHA256
...
function validateOpenSslTextPrefix(prefix) {
  ...
  const innerSignature = match(/^ {4}(?: {4})?Signature Algorithm: ([A-Za-z][A-Za-z0-9._-]{1,63})$/u);
  ...
  if (publicKeyAlgorithm[1] === 'rsaEncryption') {
    if (!match(/^ {16}(?:RSA )?Public-Key: \([1-9][0-9]{2,4} bit\)$/u)
  ...
  while (cursor < lines.length && !lines[cursor].startsWith('    Signature Algorithm:')) {
    ...
    while (cursor < lines.length
      ...
      if (header[1] === 'Basic Constraints') {
        ...
        && !/^keyid:(?:[0-9a-f]{2}:){1,63}[0-9a-f]{2}:?$/iu.test(value[1])) {
        return false;
      }
      valueLines += 1;
    }
...
async function executeTransfer({ environment, commandRunner, now, sink }) {
  ...
  try {
    ...
    result = Object.freeze({
      ...
      textPrefixSha256: validated.textPrefixSha256,
