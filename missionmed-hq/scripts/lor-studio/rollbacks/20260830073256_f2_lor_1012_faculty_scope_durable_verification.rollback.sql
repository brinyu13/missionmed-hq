-- Rollback: 20260830073256_f2_lor_1012_faculty_scope_durable_verification.rollback.sql
-- Authority: F2-LOR-1012 / DR-120 / DR-133 / DR-140
-- Date: 2026-08-30
-- Exact reverse of the durable faculty-verification resolver repair, without dependency propagation.
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION 'DR-133 faculty-scope rollback requires the exact repaired private Railway PostgreSQL identity'
      USING ERRCODE = '42501';
  END IF;
END
$identity_guard$;
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION 'DR-133 faculty-scope rollback custody mismatch'
      USING ERRCODE = '55000';
  END IF;
END
$catalog_guard$;
...
BEGIN
  ...
  IF function_count IS DISTINCT FROM 1
    ...
    OR public_execute_count <> 0
  THEN
    RAISE EXCEPTION 'DR-133 faculty-scope rollback postflight mismatch'
      USING ERRCODE = '55000';
  END IF;
END
$catalog_postflight$;
...
DECLARE
  observed_sentinel text;
  suffix text := '|facultyScopeDurableVerification=20260830073256';
BEGIN
  ...
  IF pg_catalog.right(observed_sentinel, pg_catalog.length(suffix)) IS DISTINCT FROM suffix THEN
    RAISE EXCEPTION 'DR-133 faculty-scope rollback sentinel mismatch'
      USING ERRCODE = '55000';
  END IF;

  EXECUTE pg_catalog.format(
    'COMMENT ON SCHEMA lor_studio IS %L',
    pg_catalog.left(observed_sentinel, pg_catalog.length(observed_sentinel) - pg_catalog.length(suffix))
  );
END
$restore_sentinel$;
