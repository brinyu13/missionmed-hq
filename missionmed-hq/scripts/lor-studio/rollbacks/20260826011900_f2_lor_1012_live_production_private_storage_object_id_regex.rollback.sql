-- Rollback: 20260826011900_f2_lor_1012_live_production_private_storage_object_id_regex.rollback.sql
-- Authority: F2-LOR-1012 / DR-120 / DR-133
-- Reverses: 20260826011900_f2_lor_1012_live_production_private_storage_object_id_regex.sql
-- Exact rollback: restore the hash-pinned predecessor functions and identifier constraint.
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION 'DR-133 private-storage object-id rollback requires the exact successor-bound private Railway PostgreSQL identity'
      USING ERRCODE = '42501';
  END IF;
END
$identity_guard$;
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION 'DR-133 private-storage object-id rollback catalog or data custody mismatch'
      USING ERRCODE = '55000';
  END IF;
END
$catalog_guard$;
...
DO $restore_function_definitions$
DECLARE
...
BEGIN
  ...
  LOOP
    ...
    ) / pg_catalog.length(successor_fragment);

    IF replacement_count IS DISTINCT FROM 1 THEN
      RAISE EXCEPTION 'DR-133 private-storage object-id rollback function definition drifted'
        USING ERRCODE = '55000';
    END IF;
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION 'DR-133 private-storage object-id rollback postflight mismatch'
      USING ERRCODE = '55000';
  END IF;
END
$catalog_postflight$;
...
DECLARE
  observed_sentinel text;
  suffix text := '|privateStorageObjectIdRegex=20260826011900';
BEGIN
  ...
  IF pg_catalog.right(observed_sentinel, pg_catalog.length(suffix))
      IS DISTINCT FROM suffix
  THEN
    RAISE EXCEPTION 'DR-133 private-storage object-id rollback sentinel mismatch'
      USING ERRCODE = '55000';
  END IF;
