-- Rollback: 20260825010700_f2_lor_1012_production_faculty_private_export_commands.rollback.sql
-- Authority: F2-LOR-1012 / DR-133
-- Reverses: 20260825010700_f2_lor_1012_production_faculty_private_export_commands.sql
-- Exact target: MissionMed Railway project 29afe885 / lor-staging environment f5705d38 / Postgres service b49a52e7
-- Data custody: preserves every row and refuses rollback after a faculty-private command commit
...
BEGIN
  ...
  expected_sentinel := pg_catalog.format(
    'missionmed.lor.railway-postgres-target.v1|provider=%s|project=%s|environment=%s|service=%s|database=%s|admin=%s|region=%s|decision=%s|dataCopied=%s|foundation=20260825010000|identityScope=20260825010300|facultyInvitationCommands=20260825010500|facultyPrivateExportCommands=20260825010700',
    target_provider,
    target_project_id,
    target_environment_id,
    target_service_id,
  ...
  THEN
    RAISE EXCEPTION 'DR-133 faculty-private/export rollback requires the exact successor private Railway PostgreSQL identity'
      USING ERRCODE = '42501';
  END IF;
END
$identity_guard$;
...
DECLARE
  ...
  target_provider text := pg_catalog.current_setting(
    'missionmed.lor.target_provider', true
  );
  target_project_id text := pg_catalog.current_setting(
    'missionmed.lor.target_project_id', true
  );
  target_environment_id text := pg_catalog.current_setting(
    'missionmed.lor.target_environment_id', true
  );
  target_service_id text := pg_catalog.current_setting(
    'missionmed.lor.target_service_id', true
  );
  target_database_name text := pg_catalog.current_setting(
    'missionmed.lor.target_database_name', true
  );
  target_region text := pg_catalog.current_setting(
    'missionmed.lor.target_region', true
  );
  target_decision_record text := pg_catalog.current_setting(
    'missionmed.lor.target_decision_record', true
  );
  target_data_copied text := pg_catalog.current_setting(
    'missionmed.lor.target_data_copied', true
  );
BEGIN
...
INTO STRICT
  ...
  IF table_fingerprint IS DISTINCT FROM
      '526be962fef863864d982940a8a6ad47bfad6f5341777e104e0cef63d4b276eb'
    OR function_fingerprint IS DISTINCT FROM
      '174743488bf6957e49086682fc9c9a8fa28c262f0dcb0d75953ad2e4f900acb9'
    OR policy_fingerprint IS DISTINCT FROM
      'abf9cad0a6de27fdf0fddf2819e8a206d490853c85c055c80c1f0b19a429d887'
    OR trigger_fingerprint IS DISTINCT FROM
      'cab1823e9cc012de645f8fe200489d40ea5a0202a39cd6fde509aca14c1d6039'
    OR receipt_constraint_fingerprint IS DISTINCT FROM
      'aaad35a52f81078a4d7146b255562ae3077d71e0cf18ff97ad5e9e83e98943db'
    OR grant_fingerprint IS DISTINCT FROM
      '15cf60b890b2bcd0ac3b9c4756218a5e45de813f7a7dd80dbb22930a152d1e75'
    OR dependency_fingerprint IS DISTINCT FROM
      '08134cddaea4e280f6a73ef78cf7fefe0ab720c997cbb952fbd414ae5d284fc3'
    OR metadata_fingerprint IS DISTINCT FROM
      '4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945'
    OR successor_sentinel IS DISTINCT FROM expected_successor_sentinel
  THEN
    RAISE EXCEPTION 'DR-133 faculty-private/export rollback successor semantic fingerprint mismatch'
      USING ERRCODE = '55000';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'DR-133 rollback refuses to destroy artifact export audit custody'
      USING ERRCODE = '55000';
  END IF;
  ...
  IF EXISTS (
    ...
    WHERE receipt.command_type = 'faculty.private_content_update'
  ) THEN
    RAISE EXCEPTION 'DR-133 rollback refuses to orphan faculty-private command custody'
      USING ERRCODE = '55000';
  END IF;
  ...
  INTO function_count, safe_function_count
  FROM pg_catalog.pg_proc AS procedure
  ...
  WHERE namespace.nspname = 'lor_studio'
    AND (
      (
        class.relname = 'faculty_private_content'
        AND policy.polname = 'faculty_private_content_faculty_command_insert'
        AND policy.polcmd = 'a'
      )
      OR (
        class.relname = 'released_student_documents'
        AND policy.polname =
          'released_student_documents_student_export_select'
        AND policy.polcmd = 'r'
      )
      OR (
        class.relname = 'artifact_export_audit_events'
        AND policy.polname = 'artifact_export_audit_events_command_insert'
        AND policy.polcmd = 'a'
      )
      OR (
        class.relname = 'artifact_export_audit_events'
        AND policy.polname = 'artifact_export_audit_events_command_select'
        AND policy.polcmd = 'r'
      )
    );
  ...
  IF function_count <> 3
    OR safe_function_count <> 3
    OR policy_count <> 4
    OR command_constraint NOT LIKE '%faculty.final_document_release%'
  ...
  THEN
    RAISE EXCEPTION 'DR-133 rollback successor custody mismatch'
      USING ERRCODE = '55000';
  END IF;
END
...
BEGIN
  ...
  target_policies AS (
    ...
    WHERE namespace.nspname = 'lor_studio'
      AND (
        ...
        OR class.relname = 'artifact_export_audit_events'
      )
  ),
  target_constraints AS (
    SELECT constraint_row.oid, relation.relname AS relation_name
    FROM target_relations AS relation
    JOIN pg_catalog.pg_constraint AS constraint_row
      ON constraint_row.conrelid = relation.oid
    UNION ALL
    SELECT constraint_row.oid, class.relname
    FROM pg_catalog.pg_constraint AS constraint_row
    JOIN pg_catalog.pg_class AS class ON class.oid = constraint_row.conrelid
    JOIN pg_catalog.pg_namespace AS namespace ON namespace.oid = class.relnamespace
    WHERE namespace.nspname = 'lor_studio'
  ...
  column_rows AS (
    -- The complete pg_attribute record retains attstorage, attcompression,
    -- attstattarget, attoptions, attfdwoptions, identity, and generated state.
  ...
  constraint_rows AS (
    -- The complete pg_constraint record retains PG18 contype='n' NOT NULL
    -- rows, conenforced, validation, inheritance, and action semantics.
    SELECT target.relation_name || ':' || constraint_row.conname AS sort_key,
      pg_catalog.jsonb_build_object(
    ...
    FROM target_constraints AS target
    JOIN pg_catalog.pg_constraint AS constraint_row ON constraint_row.oid = target.oid
    LEFT JOIN pg_catalog.pg_class AS referenced_class
  ...
  object_targets AS (
    ...
    SELECT 'pg_catalog.pg_constraint'::pg_catalog.regclass::oid,
      ...
      'constraint:' || target.relation_name || ':' || constraint_row.conname
    FROM target_constraints AS target
    JOIN pg_catalog.pg_constraint AS constraint_row ON constraint_row.oid = target.oid
    UNION ALL
  ...
  INTO snapshot_hash
  FROM snapshot;
  -- END ARTIFACT_ROLLBACK_CUSTODY_SNAPSHOT

  expected_snapshot_hash := CASE postgres_major
    WHEN 16 THEN 'cfa4fa8e9e13fc3dc787c8f8cf94af6e50deb205c41171e3048ce38bf93a82b7'
    WHEN 18 THEN '35fab7f464be14d86a64b05b5cb369eaa711b768fb78a864562c1ac39f60f1e0'
    ELSE NULL
  END;

  IF snapshot_hash IS DISTINCT FROM expected_snapshot_hash
    OR EXISTS (SELECT 1 FROM lor_studio.artifact_export_audit_events)
    OR EXISTS (
      SELECT 1
      FROM lor_studio.recommendation_case_private_write_receipts AS receipt
      WHERE receipt.command_type = 'faculty.private_content_update'
  ...
  THEN
    RAISE EXCEPTION 'DR-133 faculty-private/export rollback exact custody or emptiness mismatch'
      USING ERRCODE = '55000';
  END IF;
END
$exact_catalog_guard$;
...
FROM lor_studio_app;
REVOKE EXECUTE ON FUNCTION lor_studio.read_final_document_export()
FROM lor_studio_app;
REVOKE EXECUTE ON FUNCTION lor_studio.commit_faculty_private_content(
  bigint,
  jsonb,
  text,
  text,
...
DROP FUNCTION lor_studio.append_artifact_export_audit(
  jsonb,
  text,
  text,
  text
);
DROP FUNCTION lor_studio.read_final_document_export();
DROP FUNCTION lor_studio.commit_faculty_private_content(
  bigint,
  jsonb,
  text,
  text,
...
DROP POLICY faculty_private_content_faculty_command_insert
ON lor_studio.faculty_private_content;
DROP POLICY released_student_documents_student_export_select
ON lor_studio.released_student_documents;
DROP POLICY artifact_export_audit_events_command_insert
ON lor_studio.artifact_export_audit_events;
DROP POLICY artifact_export_audit_events_command_select
ON lor_studio.artifact_export_audit_events;

DROP TRIGGER artifact_export_audit_events_append_only
ON lor_studio.artifact_export_audit_events;

REVOKE SELECT, INSERT ON TABLE lor_studio.artifact_export_audit_events
FROM lor_studio_command_owner;
DROP TABLE lor_studio.artifact_export_audit_events;

REVOKE INSERT ON TABLE lor_studio.faculty_private_content
FROM lor_studio_command_owner;
...
BEGIN
  ...
  IF pg_catalog.to_regprocedure(
    ...
    OR pg_catalog.to_regclass('lor_studio.artifact_export_audit_events') IS NOT NULL
    OR pg_catalog.to_regprocedure(
      'lor_studio.commit_faculty_private_content(bigint,jsonb,text,text,jsonb,text)'
    ) IS NOT NULL
    OR command_constraint NOT LIKE '%faculty.final_document_release%'
  ...
  THEN
    RAISE EXCEPTION 'DR-133 rollback postflight mismatch'
      USING ERRCODE = '55000';
  END IF;
END
$rollback_postflight$;
