-- Rollback: 20260825010800_f2_lor_1012_ai_proposal_commands.rollback.sql
-- Authority: F2-LOR-1012 / DR-133
-- Reverses: 20260825010800_f2_lor_1012_ai_proposal_commands.sql
...
BEGIN
  ...
  expected_sentinel := pg_catalog.format(
    'missionmed.lor.disposable-postgres-harness.v1|database=%s|admin=%s|root=%s|foundation=20260820180700|identityScope=20260825010200|facultyInvitationCommands=20260825010400|facultyPrivateExportCommands=20260825010600|aiProposalCommands=20260825010800',
  ...
  THEN
    RAISE EXCEPTION 'DR-133 AI proposal command rollback requires the exact successor-bound disposable harness identity'
      USING ERRCODE = '42501';
  END IF;
END
$identity_guard$;
...
BEGIN
  ...
  EXECUTE pg_catalog.format(
    ...
    pg_catalog.regexp_replace(
      observed_sentinel, '\|aiProposalCommands=20260825010800$', ''
    )
  );
END
$restore_sentinel$;
