-- Migration: 20260826010000_f2_lor_1012_live_production_schema_foundation.sql
-- Authority: F2-LOR-1012 / DR-133
-- Date: 2026-08-26
-- Depends on: none
-- Description: Create the private LOR Studio role, schema, normalized relations, constraints, indexes, and append-only guards on an explicitly bound Railway PostgreSQL target.
-- Exact target: MissionMed Railway project 29afe885 / production environment ed3353f7 / Postgres service 576520f5. This artifact is not reusable for any other database target.
-- Idempotent: NO
...
BEGIN
  ...
  IF pg_catalog.current_setting('server_version_num')::integer / 10000
     NOT IN (16, 18) THEN
    RAISE EXCEPTION 'DR-133 foundation migration supports only PostgreSQL 16 or 18'
      USING ERRCODE = '0A000';
  END IF;
  ...
  IF target_identity_text LIKE '%mftguikkftmrxjxrkdln%'
    OR target_identity_text LIKE '%fglyvdykwgbuivikqoah%'
  THEN
    RAISE EXCEPTION 'DR-133 foundation migration target is explicitly denied'
      USING ERRCODE = '42501';
  END IF;

  IF target_deployment_environment IS DISTINCT FROM 'production'
  ...
  THEN
    RAISE EXCEPTION 'DR-133 foundation migration requires the exact private Railway PostgreSQL target identity'
      USING ERRCODE = '42501';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'DR-133 foundation migration requires a fresh lor_studio schema'
      USING ERRCODE = '42501';
  END IF;
END
$target_guard$;
...
BEGIN
  IF EXISTS (
    ...
    RAISE EXCEPTION 'LOR roles already exist; DR-133 foundation requires fresh role ownership'
      USING ERRCODE = '42710';
  END IF;
...
DO $$
DECLARE
  target_provider text := pg_catalog.current_setting('missionmed.lor.target_provider');
  target_deployment_environment text := pg_catalog.current_setting('missionmed.lor.target_deployment_environment');
  target_migration_ledger text := pg_catalog.current_setting('missionmed.lor.target_migration_ledger');
  target_project_id text := pg_catalog.current_setting('missionmed.lor.target_project_id');
  target_environment_id text := pg_catalog.current_setting('missionmed.lor.target_environment_id');
  target_service_id text := pg_catalog.current_setting('missionmed.lor.target_service_id');
  target_database_name text := pg_catalog.current_setting('missionmed.lor.target_database_name');
  target_region text := pg_catalog.current_setting('missionmed.lor.target_region');
  target_decision_record text := pg_catalog.current_setting('missionmed.lor.target_decision_record');
  target_data_copied text := pg_catalog.current_setting('missionmed.lor.target_data_copied');
  sentinel text;
BEGIN
  sentinel := 'missionmed.lor.railway-postgres-target.v2|deploymentEnvironment=production|migrationLedger=lor_studio/migrations/production'
    ...
    || '|decision=' || target_decision_record
    || '|dataCopied=' || target_data_copied
