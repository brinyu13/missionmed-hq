...
export const WORDPRESS_LOR_BOOTSTRAP_REQUEST_CONTRACT =
  'missionmed.lor.wordpress-bootstrap-redemption-request.v1';
export const WORDPRESS_LOR_BOOTSTRAP_RESPONSE_CONTRACT =
  'missionmed.lor.wordpress-bootstrap-redemption.v1';
export const WORDPRESS_LOR_ADMISSION_REQUEST_CONTRACT =
  'missionmed.lor.wordpress-admission-request.v1';
...
export const WORDPRESS_LOR_BINDING_REVOCATION_REQUEST_CONTRACT =
  'missionmed.lor.wordpress-binding-revocation-request.v1';
export const WORDPRESS_LOR_BINDING_REVOCATION_CONTRACT =
  'missionmed.lor.wordpress-binding-revocation.v1';
 ...
 * @param {{ subject: string, now: number, maximumLifetimeMs?: number }} options
 */
...
function assertBootstrapResponse(raw, { now }) {
  ...
  const receipt = assertWordPressLorAdmissionReceipt(raw.receipt, { subject, now });
...
function assertBindingRevocation(raw, { bindingId, subject, now }) {
...
export function createWordPressLorS2sClient({
  ...
  return Object.freeze({
    /** @param {{ code?: unknown, state?: unknown, callback?: unknown }} [input] */
    async redeemBootstrap({ code, state, callback } = {}) {
      ...
      return assertBootstrapResponse(raw, { now: nowMilliseconds(clock) });
    },

    /** @param {{ bindingId?: unknown, subject?: unknown }} [input] */
    async admit({ bindingId, subject } = {}) {
      const exactBindingId = exactPattern(bindingId, BINDING_PATTERN, 'BINDING_INVALID');
      const exactAuthenticatedSubject = exactSubject(subject);
    ...
    /** @param {{ bindingId?: unknown, subject?: unknown }} [input] */
    async revokeBinding({ bindingId, subject } = {}) {
      const exactBindingId = exactPattern(bindingId, BINDING_PATTERN, 'BINDING_INVALID');
      const exactAuthenticatedSubject = exactSubject(subject);
