...
const CANONICAL_ADMIN_URL = 'https://missionmedinstitute.com/usce-admin/';
...
async function sendPublicIntakeNotifications({ requestId, data, submittedAt, status, wasExisting }) {
  ...
  const adminMessage = buildAdminRequestNotification({ requestId, data, submittedAt, status, wasExisting });
...
function buildAdminRequestNotification({ requestId, data, submittedAt, status, wasExisting }) {
  const subject = `New MissionMed USCE request: ${data.student_name}`;
  const timestamp = formatAdminSubmittedAt(submittedAt);
  const locations = sanitizeStringArray(data.preferred_locations, 8, 120);
  const specialties = sanitizeStringArray(data.preferred_specialties, 8, 120);
  const dates = sanitizeStringArray(data.preferred_months_or_dates, 10, 120);
  const rows = [
    ['Reference', requestId],
    ['Status', status || 'new'],
    ['Submission timestamp', timestamp],
    ['Student name', data.student_name],
    ['Student email address', data.email],
  ...
  const textBody = [
    'New USCE request received',
    '',
    ...rows.map(([label, value]) => `${label}: ${value}`),
    '',
    'Requested locations, ranked order:',
    formatAdminRankedPlain(locations),
    '',
    'Requested specialties:',
    formatAdminRankedPlain(specialties),
    '',
    'Requested dates / availability dates:',
    formatAdminRankedPlain(dates),
    '',
    `Open USCE Admin: ${CANONICAL_ADMIN_URL}`,
  ].join('\n');

  const detailRows = rows.map(([label, value]) => adminDetailRowHtml(label, value)).join('');
  const htmlBody = [
    '<!doctype html><html><body style="margin:0;padding:0;background:#f6f4ef;color:#0f2a44;font-family:Arial,Helvetica,sans-serif;">',
    '<div style="display:none;max-height:0;overflow:hidden;opacity:0;color:transparent;">A new USCE request is ready for admin review.</div>',
    '<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#f6f4ef;"><tr><td align="center" style="padding:28px 16px;">',
    '<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:720px;background:#ffffff;border:1px solid #ded7c6;border-radius:8px;overflow:hidden;">',
    '<tr><td style="padding:22px 24px;background:#0f2a44;color:#ffffff;border-bottom:1px solid #eadfca;">',
    '<div style="font-size:12px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:#d9b85b;">MissionMed USCE</div>',
    '<h1 style="margin:8px 0 0;font-size:24px;line-height:1.25;font-weight:700;color:#ffffff;">New USCE Request</h1>',
    '</td></tr>',
    '<tr><td style="padding:22px 24px;">',
    '<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;margin:0 0 22px;">',
    detailRows,
    '</table>',
    adminSectionHtml('Requested locations, ranked order', formatAdminRankedHtml(locations)),
    adminSectionHtml('Requested specialties', formatAdminRankedHtml(specialties)),
    adminSectionHtml('Requested dates / availability dates', formatAdminRankedHtml(dates)),
    '<div style="margin-top:26px;">',
    `<a href="${escapeHtml(CANONICAL_ADMIN_URL)}" style="display:inline-block;background:#d9b85b;color:#0f2a44;text-decoration:none;font-weight:700;border-radius:6px;padding:13px 18px;">Open USCE Admin</a>`,
    '</div>',
    '</td></tr>',
    '</table>',
    '</td></tr></table>',
...
function adminDetailRowHtml(label, value) {
  return [
    '<tr>',
    '<td style="width:210px;padding:8px 10px 8px 0;border-bottom:1px solid #eee6d7;color:#647184;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;vertical-align:top;">',
    escapeHtml(label),
    '</td>',
    '<td style="padding:8px 0;border-bottom:1px solid #eee6d7;color:#0f2a44;font-size:14px;line-height:1.5;vertical-align:top;">',
    escapeHtml(value),
    '</td>',
    '</tr>',
  ].join('');
...
function adminSectionHtml(title, bodyHtml) {
  return [
    '<div style="margin:0 0 20px;">',
    '<h2 style="margin:0 0 8px;color:#0f2a44;font-size:15px;line-height:1.35;">',
    escapeHtml(title),
    '</h2>',
    bodyHtml,
    '</div>',
...
function formatAdminRankedPlain(items) {
  if (!items.length) return 'Not provided';
  return items.map((item, index) => `${index + 1}. ${item}`).join('\n');
}

function formatAdminRankedHtml(items) {
  if (!items.length) {
    return '<p style="margin:0;color:#647184;font-size:14px;line-height:1.5;">Not provided</p>';
  }

  return [
    '<ol style="margin:0;padding-left:22px;color:#0f2a44;font-size:14px;line-height:1.65;">',
