# MissionMed File Vault · Program-Specific PS

An isolated Program-Specific Personal Statement workflow for administrators and currently entitled MissionMed 360 members. Governing architecture: PSV-0002, DR-327 and DR-331.

- **Owns nothing it should not.** File Vault owns PS documents; RISE owns program intelligence. This module reads both, writes to neither, and keeps its outputs in eleven isolated, namespaced tables.
...
| `includes/class-mmps-gate.php` | kill switches, administrator/current-360 entitlement validation, REST permission (404 outside authorized access) |
| `includes/class-mmps-install.php`, `class-mmps-store.php` | eleven namespaced tables; owner-scoped reads and writes; transactional persistence; audit of ids and hashes only |
...
| `includes/class-mmps-root-source.php`, `class-mmps-fv-reader.php`, `class-mmps-docx.php` | ROOT sources: File Vault (read-only, reflection-checked), direct DOCX/TXT upload, pasted text, and an internal synthetic diagnostic |
...
| `includes/class-mmps-provider.php`, `class-mmps-generator.php` | dedicated PSV AI adapter (store:false, strict schema), complete-ROOT read-only context, five-strategy candidate set, exact-region output, deterministic evidence/diversity validators |
...
Production wp-config constants: `MMED_PS_PROTO_OPENAI_API_KEY`, optional `MMED_PS_PROTO_OPENAI_MODEL`, and `MMED_PS_PROTO_DISABLE` (hard kill switch). `MMED_PS_PROTO_ALLOW_REAL_ROOT_AI` remains undefined under DR-331. `MMED_PS_PROTO_ALLOW_SIMULATOR`, testing endpoints, the legacy allowlist constant, and legacy canary tuple constants are diagnostic/historical only and must not be used for the normal student path.
