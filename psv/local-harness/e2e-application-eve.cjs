...
(async () => {
	...
	ok('canonical ERAS manifest includes only approved owner statements', response.status === 200 && manifest.schema === 'missionmed.psv.eras-assignment-manifest.v2' && manifest.items.length >= 1 && manifest.items.every(item => item.approvalStatus === 'APPROVED'));
	ok('manifest keeps ACGME, specialty, training type, NRMP track and MyERAS resolution separate without guessing', manifest.items.every(item => item.acgmeId && item.programSpecialtyId && item.specialty && item.statementTitle && item.exportFilename && ['RESOLVED','AMBIGUOUS','UNAVAILABLE'].includes(item.trainingTypeStatus) && ['RESOLVED','AMBIGUOUS','UNAVAILABLE'].includes(item.nrmpTrackStatus) && item.myErasIdentity === null && item.myErasIdentityStatus === 'UNRESOLVED'));
	ok('unresolved or ambiguous identity rows fail closed instead of appearing assignment-ready', manifest.items.every(item => item.assignmentEligible === false && item.assignmentStatus === 'NEEDS_ATTENTION' && item.identityAttentionReasons.includes('MYERAS_IDENTITY_UNRESOLVED')));
	ok('manifest provides a separate deterministic MyERAS-safe title and both original and ERAS-normalized hashes', manifest.items.every(item => item.myErasTitle && item.myErasTitle.length <= 50 && item.myErasTitleMaxCharacters === 50 && /^[a-f0-9]{64}$/.test(item.fullTextSha256) && /^[a-f0-9]{64}$/.test(item.erasNormalizedTextSha256) && /curly-apostrophes-to-ascii/.test(item.erasNormalizationRule)));
