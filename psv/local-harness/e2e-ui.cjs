...
const SHOTS = '/home/claude/wpdev/harness/shots';
...
(async () => {
	...
	ok('preview: ROOT check Unchanged + facts with sources', (await page.locator('.tag:has-text("Unchanged")').count()) === 1 && (await page.locator('.fact.used').count()) >= 3 && (await page.locator('.factSrc a').count()) >= 2);
	...
	fs.writeFileSync('/home/claude/wpdev/harness/e2e-ui-results.json', JSON.stringify({ results, problems }, null, 1));
