...
check(!is_wp_error(MMPS_Edit::read(1,$uuid)),'entitled production user can edit only the confirmed real-ROOT region');
MMPS_Gate::$allow=false;
check(is_wp_error(MMPS_Edit::read(1,$uuid)),'non-entitled real-ROOT edit access fails closed');
MMPS_Gate::$allow=true;
MMPS_Store::$roots[1][$root['id']]=$root;
...
// A fictional real ROOT exercises the normal DR-331 production path; no real-student prose fixture.
MMPS_Store::$roots[1][$root['id']]['isSynthetic']=false;
MMPS_Store::$runs[1][$uuid]['program_specialty_id']='rise_ps_production_fixture';
$productionHeads=MMPS_Edit::read(1,$uuid);
check(!is_wp_error($productionHeads) && $productionHeads['capabilities']['canEdit'] && $productionHeads['capabilities']['canApprove'],'entitled real ROOT supports normal edit and approval capability');
$productionSave=$restore; $productionSave['baseRevisionId']=$restored['revision']['id']; $productionSave['requestId']=MMPS_Store::uuid();
$productionSaved=MMPS_Edit::write(1,$uuid,$productionSave);
check(!is_wp_error($productionSaved) && $productionSaved['revision']['validation']['canApprove'],'restored exact production revision is approval eligible');
check('REAL_ROOT_PRODUCTION'===MMPS_Provider::authorization_mode_for(1,MMPS_Store::$roots[1][$root['id']],'rise_ps_other_fixture'),'second program uses the same normal production authority');
check(!defined('MMED_PS_PROTO_ALLOW_REAL_ROOT_AI'),'legacy broad constant remains unnecessary and undefined');
MMPS_Store::$roots[1][$root['id']]=$root;
...
check(MMPS_Store::$lockCalls>0,'edit and library writes enter shared atomic boundary');
