...
m3check(str_contains($src['batch'], 'mmps_batch_transition_commit') && str_contains($src['batch'], 'mmps_batch_requeue_persist') && str_contains($src['batch'], 'mmps_batch_approval_persist') && str_contains($src['batch'], 'mmps_batch_refresh_persist'), 'all item transitions and derived counters fail closed', 'Requeue, approval, slot release, stale recovery and counter refresh writes must be checked.');
m3check(str_contains($src['batch'], 'mmed_ps_proto_test_fail_transition_commit') && str_contains($src['batch'], 'MMPS_Gate::testing()'), 'local harness can inject a transition commit failure without a production control', 'Recovery and idempotency need an executable failure-path test.');
m3check(!str_contains($src['batch'], "'active_items' => min") && str_contains($src['batch'], 'GREATEST(active_items-%d,0)'), 'stale recovery decrements only the slots it owns', 'A stale-worker recount must not overwrite a concurrent claim.');
m3check(str_contains($src['gen'], 'mmps_run_store') && str_contains($src['batch'], 'mmps_batch_transition_persist'), 'run and item persistence failures fail closed', 'READY may only reference a durably stored run.');
m3check(str_contains($src['store'], 'find_document_by_run_revision') && str_contains($src['rest'], "'candidateId'") && str_contains($src['rest'], '$candidate_id') && str_contains($src['rest'], "'editRevisionId'"), 'saved alternatives and edited revisions retain immutable provenance', 'A later version may coexist, but it cannot relabel or overwrite an earlier candidate revision.');
...
m3check(str_contains($src['ui'], 'docUuids: allApproved ? [] : ids'), 'Download All ignores checkbox selection', 'Download All must export every approved owner document even when rows are selected.');
