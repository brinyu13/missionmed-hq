...
c(str_contains($files['ui'],"Promise.all([api('GET','/roots/'+doc.rootId),api('GET','/runs/'+runId)])")&&str_contains($files['ui'],'S.selected=[programId]')&&str_contains($files['ui'],'programId!==doc.programSpecialtyId'),'library edit rebinds the exact ROOT and verified program before review');
...
c(str_contains($files['rest'],'LATEST_APPROVED_PER_PROGRAM_TRAINING_TYPE_AND_NRMP_TRACK')&&str_contains($files['rest'],'supersededApprovedVersions'),'MyERAS preparation selects one current approved version per program, training type and NRMP track and preserves older versions');
