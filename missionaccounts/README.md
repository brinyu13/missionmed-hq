...
The Zoom provider port is `POST /api/internal/zoom/sync`, also worker-token authenticated and independently gated by `MISSIONACCOUNTS_ZOOM_SYNC=1`. The default provider is deliberately disconnected. An authorized runtime must inject a configured `ZoomAttendanceProvider` client and a meeting classifier; successful runs persist immutable artifact/import/session/participant-source evidence and operational health but create zero identity decisions, attendance interpretations, billing decisions, or charges. Failed configured-provider runs persist a failed sync and one review exception. No Zoom credentials, schedule, or live API client are included or inferred here.
