...
The PostgreSQL check applies the complete migration to a temporary, local-only PostgreSQL cluster and exercises billing, attendance correction, exam, comp, Stripe payment setup, automatic-billing consent, one-charge-per-attendance-day, and notification-outbox transactions. The build command verifies the Founder-approved 5300A SHA-256 before materializing the browser artifact. The generated HTML is intentionally gitignored because the canonical payload contains historical student information. Local auth is development-only and fails closed in production.
