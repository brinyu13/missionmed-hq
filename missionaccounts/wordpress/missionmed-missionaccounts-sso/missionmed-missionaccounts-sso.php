 ...
 * Description: Default-off WordPress session bridge, exact-user entitlement gate, and Matrix navigation seam for MissionAccounts.
 * Version: 0.1.0
...
function mma_role_for_user($user, $settings = null) {
    ...
    return in_array($override, array('student', 'missionaccounts_admin', 'founder'), true)
        ? $override
        : $native;
}
...
function mma_access_state($user) {
    ...
    return array('role' => $role, 'entitlement' => $entitlement);
}
...
function mma_dashboard_tile_item($tiles) {
    ...
    $tiles[] = array(
        'id' => 'missionaccounts',
        'label' => 'MissionAccounts',
        'subtitle' => 'Attendance, billing, and exam progress',
        'url' => home_url(mma_settings()['base_path']),
    );
    return $tiles;
}
...
function mma_menu_items($items, $args) {
    ...
    return $items . sprintf(
        ...
        esc_html__('MissionAccounts', 'missionmed')
    );
}
