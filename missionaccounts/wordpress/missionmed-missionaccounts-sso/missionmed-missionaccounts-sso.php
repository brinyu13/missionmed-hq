...
function mma_inject_matrix_module() {
    ...
    echo '<script id="missionmed-missionaccounts-inline-launch">(function(){"use strict";var module=' . wp_json_encode($module) . ',target=' . wp_json_encode($target) . ';function mount(){var o=window.MMED_OS;if(o&&Array.isArray(o.modules)&&!o.modules.some(function(m){return m.id==="missionaccounts";})){o.modules.push(module);}if(document.querySelector("[data-missionaccounts-entry=nav]")){return true;}var list=null;Array.prototype.slice.call(document.querySelectorAll(".sos-nav-section")).some(function(section){var label=section.querySelector(".sos-nav-label");if(label&&/account/i.test(label.textContent||"")){list=section.querySelector(".sos-nav-list");return !!list;}return false;});if(!list){list=document.querySelector("#sos-sidebar .sos-nav-list");}if(!list){return false;}var item=document.createElement("li"),link=document.createElement("a"),icon=document.createElement("span"),label=document.createElement("span");item.setAttribute("data-missionaccounts-entry","nav");link.className="sos-nav-link missionaccounts-nav-link";link.setAttribute("data-missionaccounts-launch","");link.href=target;icon.className="sos-nav-icon";icon.textContent="MA";label.textContent="MyMissionMed Account";link.appendChild(icon);link.appendChild(label);item.appendChild(link);list.appendChild(item);return true;}function boot(){mount();if("MutationObserver" in window){new MutationObserver(mount).observe(document.documentElement,{childList:true,subtree:true});}[250,1000,3000,6000].forEach(function(delay){window.setTimeout(mount,delay);});}document.addEventListener("click",function(event){var launch=event.target&&event.target.closest?event.target.closest("[data-missionaccounts-launch]"):null;if(!launch){return;}event.preventDefault();event.stopPropagation();window.location.assign(target);},true);if(document.readyState==="loading"){document.addEventListener("DOMContentLoaded",boot);}else{boot();}})();</script>' . "\n";
}
