...
export const ISO_COUNTRY_CODES = Object.freeze(ISO_COUNTRY_CODE_TEXT.trim().split(/\s+/));
export const ISO_COUNTRY_CODE_SET = new Set(ISO_COUNTRY_CODES);

export function isIsoCountryCode(value) {
  return ISO_COUNTRY_CODE_SET.has(String(value || '').trim().toUpperCase());
}
...
export function countryOptions(locale = 'en') {
  ...
  return ISO_COUNTRY_CODES
    ...
    .sort((left, right) => left.code === 'US' ? -1 : right.code === 'US' ? 1 : left.name.localeCompare(right.name));
}

if (typeof window !== 'undefined') {
  window.MissionAccountsCountries = Object.freeze({
    codes: ISO_COUNTRY_CODES,
    isValid: isIsoCountryCode,
