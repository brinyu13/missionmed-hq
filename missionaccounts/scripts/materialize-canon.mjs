...
const productionActionGuards = [
  ...
  ["function submitExam(si, step, date, by){", "function submitExam(si, step, date, by){ if(document.documentElement.dataset.missionaccountsBuild==='production') return by==='admin'?window.MissionAccountsRuntime.dispatch('unsupported',{message:'Administrator exam entry is not enabled yet.'}):window.MissionAccountsRuntime.dispatch('student-exam-submit',{si,step,date});"],
