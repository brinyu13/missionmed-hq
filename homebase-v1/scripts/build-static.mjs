...
const developmentMarker = Buffer.from(`${JSON.stringify({
  mode: 'development',
  deployable: false,
  warning: 'HomeBase development output is not a release artifact.',
}, null, 2)}\n`);
...
function replaceExactlyOnce(source, needle, replacement, label) {
  ...
  if (pieces.length !== 2) {
    throw new Error(`Expected exactly one ${label} reference in the HomeBase source.`);
  }
...
if (rewrittenApp.includes('missionmed-logo.png')) {
  throw new Error('HomeBase runtime code must not inject an unmanifested MissionMed logo path.');
}
...
const head = mode === 'release'
  ...
  : [
    '<head>',
    '  <base href="/">',
    '  <meta name="homebase-build-mode" content="development-only">',
  ].join('\n');
...
console.log(JSON.stringify({
  ...
  warning: mode === 'release'
    ...
    : 'Development-only output cannot be used as a HomeBase release.',
  outputDirectory: path.relative(packageDir, distDir),
  basePath: mode === 'release' ? '/homebase/' : '/',
  index: 'index.html',
  assets: [
    appName,
    authName,
