...
const routeFile = path.join(wordpressDir, 'missionmed-homebase-route.php');
const runtimeRoot = path.join(wordpressDir, 'missionmed-homebase-runtime');
...
if (mode !== 'release') {
  throw new Error(
    'HomeBase release provenance failed: WordPress artifacts require --mode=release.',
  );
}
...
const manifestStartMarker = '\t\t// BEGIN GENERATED HOMEBASE ASSET MANIFEST.';
const manifestEndMarker = '\t\t// END GENERATED HOMEBASE ASSET MANIFEST.';
const releaseStartMarker = '// BEGIN GENERATED HOMEBASE RELEASE ID.';
const releaseEndMarker = '// END GENERATED HOMEBASE RELEASE ID.';
...
async function filesBelow(directory, prefix = '') {
  ...
  for (const entry of directoryEntries.sort((a, b) => a.name.localeCompare(b.name))) {
    ...
    if (entry.isDirectory()) {
      ...
      throw new Error(`Unsupported HomeBase release filesystem entry: ${relative}.`);
    }
  }
  return output;
}
...
function replaceMarkedBlock(source, startMarker, endMarker, replacement) {
  ...
  if (start === -1 || end === -1 || end < start) {
    throw new Error(`HomeBase route marker block is missing or invalid: ${startMarker}`);
  }
...
if (files.length !== approvedTopology.length) {
  throw new Error(`Expected the approved ${approvedTopology.length}-file HomeBase release, found ${files.length}.`);
}
...
for (const relative of files) {
  ...
  if (matches.length !== 1) {
    throw new Error(`Unexpected HomeBase release path: ${relative}.`);
  }
}
...
if (aliases.size !== entries.length) {
  throw new Error('The approved HomeBase release contains a 12-character SHA alias collision.');
}
...
if (
  ...
  throw new Error('Built HomeBase HTML does not use only the approved extensionless asset aliases.');
}
if (
  !appText.includes(`from './${authEntry.alias}'`)
  || /from ['"].*auth\.[a-f0-9]{12}\.js['"]/.test(appText)
) {
  throw new Error('Built HomeBase app module does not use the approved extensionless auth alias.');
}
for (const fontEntry of fontEntries) {
  if (!stylesText.includes(`url("./${fontEntry.alias}")`)) {
    throw new Error(`Built HomeBase stylesheet does not use the alias for ${fontEntry.path}.`);
  }
}
if (stylesText.includes('./fonts/')) {
  throw new Error('Built HomeBase stylesheet still contains an extension-bearing font URL.');
}
...
if (checkOnly) {
  if (next !== source) {
    throw new Error('HomeBase WordPress route manifest is stale; run npm run build:release.');
  }
  let releaseEntries;
  try {
    releaseEntries = await readdir(runtimeRoot, { withFileTypes: true });
  } catch {
    throw new Error('HomeBase WordPress release bundle is missing; run npm run build:release.');
  }
  if (
    ...
    throw new Error('HomeBase WordPress release source is stale; run npm run build:release.');
  }
  let existingBundle;
  try {
    existingBundle = await readFile(bundleFile, 'utf8');
  } catch {
    throw new Error('HomeBase WordPress release bundle is incomplete; run npm run build:release.');
  }
  if (existingBundle !== bundleSource) {
    throw new Error('HomeBase WordPress release bundle bytes are stale; run npm run build:release.');
  }
  let existingEdgeAliases;
  try {
    existingEdgeAliases = await readFile(edgeAliasFile, 'utf8');
  } catch {
    throw new Error('HomeBase edge alias manifest is missing; run npm run build:release.');
  }
  if (existingEdgeAliases !== edgeAliasSource) {
    throw new Error('HomeBase edge alias manifest is stale; run npm run build:release.');
  }
  ...
  console.log(
    `HomeBase WordPress route and ${releaseId} release.php match commit `
  ...
  const sourceLabel = completedReleaseProof.clean
    ...
    : '';
  console.log(
    `HomeBase WordPress route and ${releaseId} release.php generated as a candidate`
