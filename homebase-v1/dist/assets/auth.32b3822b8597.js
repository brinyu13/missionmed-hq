...
export async function boundedFetch(input, init = {}, timeoutMs = requestTimeoutMs) {
  ...
  const timer = globalThis.setTimeout(() => controller.abort('homebase_request_timeout'), timeoutMs);
  try {
    return await fetch(input, { ...init, signal: controller.signal });
  } catch (error) {
    if (controller.signal.aborted && !upstreamSignal?.aborted) {
      const timeout = new Error('HomeBase took too long to respond. Return to Matrix and try again.');
...
export function createAuthClient({ onLockout = () => {} } = {}) {
  ...
  async function bootstrapBridge({ redirectOnUnauthenticated = false } = {}) {
    const endpoint = new URL(
      config.wpBootstrapPath || '/wp-admin/admin-ajax.php?action=missionmed_homebase_bootstrap',
    ...
    bridgeNonce = String(payload.data?.nonce || '');
    return payload.data;
  }
  ...
  async function performExchange() {
    ...
    if (payload.nonce) bridgeNonce = String(payload.nonce);
  ...
  // Same-origin WordPress REST call (Matrix Calendar, File Vault V2). Uses the
  // logged-in WordPress cookie session plus the bridge nonce — never the
  // HomeBase bearer token.
  async function wpRequest(pathname, options = {}) {
    if (config.devAuth) {
      const error = new Error('Matrix services are unavailable in local fixture mode.');
      error.code = 'wp_unavailable';
      error.status = 503;
      throw error;
    }
    if (!bridgeNonce) await bootstrapBridge({});
    const response = await boundedFetch(new URL(pathname, window.location.origin), {
      ...options,
      credentials: 'include',
      cache: 'no-store',
      headers: {
        Accept: 'application/json',
        'X-WP-Nonce': bridgeNonce,
        ...(options.headers || {}),
      },
    });
    const payload = await readJson(response);
