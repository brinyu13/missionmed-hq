...
(function b1513Shim() {
  ...
  const INVITATIONS = {
    'u-founder': [
      { id: 'inv-1', token: 'rs-demo-rosa', contributorName: 'Rosa', relationship: 'parent', emailMasked: 'r•••@gmail.com', status: 'contributed', sentAt: days(9), openedAt: days(8), contributedAt: days(7), expiresAt: days(-21), remindersSent: 1, videoGreeting: { state: 'recorded', durationMs: 21000 }, disclosureVersion: 'guest-contribution-1', personalMessage: 'Mom — I’m collecting stories for my residency applications. Anything you remember counts.' },
      { id: 'inv-2', token: 'rs-demo-ken', contributorName: 'Dr. Ito', relationship: 'mentor', emailMasked: 'k•••@univ.edu', status: 'opened', sentAt: days(4), openedAt: days(2), expiresAt: days(-26), remindersSent: 0, videoGreeting: null, disclosureVersion: 'guest-contribution-1', personalMessage: '' },
      { id: 'inv-3', token: 'rs-demo-sam', contributorName: 'Sam', relationship: 'best_friend', emailMasked: 's•••@outlook.com', status: 'sent', sentAt: days(1), expiresAt: days(-29), remindersSent: 0, videoGreeting: null, disclosureVersion: 'guest-contribution-1', personalMessage: '' },
    ],
  };
  const CONTRIBUTIONS = {
    'u-founder': [
  ...
  async function routeR(method, path, body, init) {
    ...
    if (guestMatch) {
      ...
      const prompts = (CONTRIB_LIBRARY?.prompts || []).filter((prompt) => prompt.rel.includes(invitation.relationship))
    ...
    if (invMatch) {
      ...
      if (op === 'send' || op === 'resend') {
        invitation.status = invitation.status === 'contributed' ? 'contributed' : 'sent';
        invitation.sentAt = new Date().toISOString();
        if (op === 'resend') invitation.remindersSent += 1;
        return json({ invitation, delivery: 'simulated-delivered' });
      }
      if (op === 'revoke') { invitation.status = 'revoked'; return json({ invitation }); }
      if (op === 'email-preview') {
        const student = me;
        return json({ preview: {
          ...
          note: 'Exact external wording requires Founder approval (FD-R2) before any real email is sent.',
        } });
      }
    ...
    if (path.includes('/api/inspiration/browse')) {
      ...
      return json({ prompts: prompts.map((prompt) => ({ ...prompt, favorite: favorites.has(prompt.id) })), total: prompts.length, favoriteCount: favorites.size });
    }
