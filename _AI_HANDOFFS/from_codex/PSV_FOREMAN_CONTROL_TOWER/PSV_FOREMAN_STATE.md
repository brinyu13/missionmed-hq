...
- P1 local gates: all 17 PHP contract/runtime scripts PASS, including P1 contract 17/17, calibration contract 9/9, P1 runtime 9/9 and explicit 20,000/20,001 ms aggregate retry-boundary checks; plugin PHP lint 22/22, JavaScript parse and `git diff --check` PASS; disposable WordPress API acceptance 131/131 PASS. The first exact-commit verifier correctly blocked the 70-second provider timeout paired with the old 40-second retry-start threshold. The repair now exposes one provider timeout constant and starts a second request only when its full 70 seconds plus 5 seconds overhead fit within a 95-second edge budget. Production remains v1.0.1 and unmodified pending repaired commit verification, package/preflight and narrow PATH lease.
