...
At 00:29 UTC, after successful acceptance, Kinsta logged a site-wide PHP-FPM saturation event. The RISE SSO action returned client-closed HTTP 499 after 180 seconds while unrelated WordPress REST, Matrix/Scheduler, MissionAccounts, and cron requests simultaneously timed out. MyKinsta reported the four-thread PHP limit reached 104 times. A final 20-second replay in that window still timed out without response bytes from WordPress login and the public `/rise/` proxy while the direct isolated RISE health endpoint remained HTTP 200. This was external operational evidence, not an attributable 5011 regression.

On September 9 the shared pool recovered without a RISE, WordPress, or Kinsta mutation from this ticket. Fresh readback showed WordPress login HTTP 200, public `/rise/` expected anonymous 302, and direct RISE health HTTP 200. A new authorized 360 end-to-end replay reconfirmed all required filters and features. Three consecutive Matrix profile reads returned HTTP 200, and the dedicated settled mobile replay passed at 390 by 844. The temporary-session cleanup readbacks were false. The post-acceptance infrastructure incident is therefore closed for P1-RISE-5011 with no attributable regression and no remaining human gate.
