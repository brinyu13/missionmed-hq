...
foreach ( $orders as $order ) {
	...
	$notes     = wc_get_order_notes( array( 'order_id' => $order->get_id(), 'limit' => 100 ) );
	...
	$entry     = array(
		...
		'stripe_owner'           => (string) $order->get_meta( '_missionmed_stripe_owner', true ),
		'stripe_route'           => (string) $order->get_meta( '_missionmed_stripe_router', true ),
		'coupon_codes'           => $codes,
	...
	if ( function_exists( 'wcs_get_subscriptions_for_order' ) ) {
		foreach ( wcs_get_subscriptions_for_order( $order->get_id(), array( 'order_type' => 'parent' ) ) as $subscription ) {
