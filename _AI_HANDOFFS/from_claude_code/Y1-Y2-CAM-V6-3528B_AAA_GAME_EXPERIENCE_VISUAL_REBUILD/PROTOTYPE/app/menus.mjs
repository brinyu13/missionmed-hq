...
import { CATEGORIES, QUESTIONS, SESSIONS, STUDENT, CALIBRATION, paintSimFeed, SimEngine } from './data.mjs';
...
/* live simulated preview feed on a canvas */
function mountSimFeed(canvas, engineRef = null) {
  const ctx = canvas.getContext('2d');
  let raf, t0 = performance.now();
  const loop = () => {
    raf = requestAnimationFrame(loop);
    const t = (performance.now() - t0) / 1000;
    const w = canvas.width = canvas.clientWidth * Math.min(devicePixelRatio, 1.5);
    const h = canvas.height = canvas.clientHeight * Math.min(devicePixelRatio, 1.5);
    if (w && h) paintSimFeed(ctx, w, h, t, engineRef ? engineRef.frame() : { speaking: true });
  };
  loop();
...
function homeScreen(el) {
  ...
  <div class="topline">
    ...
    <button class="chip" data-focusable data-continue>${svgIcon('M7 4.5v15l12-7.5z', 13)} CONTINUE · ${latest.title.toUpperCase()}</button>
  </div>
  <div class="home-body">
    <div class="home-title">
      <div class="home-kicker">YOUR ARENA IS OPEN</div>
      <h1 class="t-display">READY TO TRAIN,<br>${STUDENT.name.toUpperCase()}?</h1>
  ...
  <div class="lobby-row" role="list">
    ...
    <button class="lobby-card sec" data-focusable data-go="library" aria-label="Video Library">
      ...
      <span class="lc-count">${SESSIONS.length}</span>
    </button>
  ...
  el.addEventListener('click', e => {
    ...
    if (e.target.closest('[data-continue]')) { draft.mode = 'question'; draft.qids = [latest.qid]; saveDraft(); go('setup'); }
  });
}
...
export function startMode(mode) {
  draft.mode = mode;
  if (mode === 'quick') {
    const pick = QUESTIONS.find(q => q.practiced === 0) || QUESTIONS[0];
    draft.qids = [pick.id];
    saveDraft();
    go('setup');
  } else {
...
function questionsScreen(el) {
  ...
  function renderGrid() {
    ...
    gridEl.innerHTML = visible().map((q, i) => {
      ...
      <button class="qcard ${on ? 'sel' : ''} ${previewId === q.id ? 'pv' : ''}" data-focusable ${i === 0 ? 'data-autofocus' : ''} data-q="${q.id}" style="--cc:${c.color}">
        ...
        <span class="qc-foot">
          ${q.practiced ? `<span>practiced ×${q.practiced}${q.best ? ` · best ${q.best.toFixed(1)}` : ''}</span>` : '<span class="new">not practiced yet</span>'}
  ...
  function renderPreview() {
    ...
    prevEl.innerHTML = `
      ...
      <div class="qp-meta">
        <span><em>PRACTICED</em><b>${q.practiced || '—'}</b></span>
        <span><em>PERSONAL BEST</em><b>${q.best ? q.best.toFixed(1) : '—'}</b></span>
...
function setupScreen(el) {
  ...
  draft.cam = draft.cam || 'Simulated FaceTime HD (prototype)';
  draft.mic = draft.mic || 'Simulated Studio Mic (prototype)';
  saveDraft();
  ...
  <div class="topline">
    ...
    <span class="spacer"></span>
    <span class="chip amber"><span class="dot"></span>SIMULATED DEVICES · PROTOTYPE</span>
  </div>
    ...
    <section class="setup-left">
      <div class="panel">
        ...
        <div class="setup-feed">
          <canvas id="setupFeed"></canvas>
          <span class="feed-tag">LIVE PREVIEW · SIMULATED FEED</span>
  ...
  const stopFeed = mountSimFeed(el.querySelector('#setupFeed'));
  const micFill = el.querySelector('#micFill');
  let micRaf, mt = 0;
  const micLoop = () => { micRaf = requestAnimationFrame(micLoop); mt += .04; micFill.style.width = `${34 + Math.abs(Math.sin(mt * 1.7)) * 44 + Math.sin(mt * 7.3) * 8}%`; };
  micLoop();
  ...
  el.addEventListener('click', async e => {
    ...
    if (wheel) { toast('Prototype uses the simulated device — Codex wires real enumerateDevices().', 'info'); return; }
  ...
  return { destroy: () => { stopFeed(); cancelAnimationFrame(micRaf); } };
}

/* ================= READY — frame up, then go ================= */
function readyScreen(el) {
  ...
  <div class="ready-body">
    ...
    <aside class="ready-side">
      ...
      <button class="btn btn-gold btn-xl ready-start" data-focusable data-autofocus data-start>▸ START INTERVIEW</button>
      <div class="ready-note">Recording and analytics arm together — one gesture, t = 0. Esc opens the pause menu at any time.</div>
      <button class="btn btn-quiet" data-focusable data-back>◂ BACK</button>
    </aside>
  </div>
  <div class="countdown" id="countdown" hidden><b></b></div>`;

  const stopFeed = mountSimFeed(el.querySelector('#readyFeed'));
  let counting = false;
  el.addEventListener('click', e => {
...
function recoveryScreen(el) {
  ...
  el.addEventListener('click', async e => {
    if (e.target.closest('[data-save]')) { toast('Partial session saved to your library (prototype).', 'save'); go('results'); }
