...
// Local, memory-only, provider-free ASR. The worker loads only vendored assets;
// it receives bounded PCM windows and returns timing aggregates without text.
...
for (const signal of ['SIGINT', 'SIGTERM']) {
  process.on(signal, async () => {
    if (stopping) return;
    stopping = true;
    await handler.shutdown('harness_shutdown');
