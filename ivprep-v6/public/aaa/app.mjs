...
function navigate(view, { focus = true, updateHash = true } = {}) {
  ...
  if (view !== "room" && state.roomStarted) stopPrototypeRoom({ results: false });
  state.view = view;
  document.body.dataset.activeView = view;
...
function startPrototypeRoom() {
  ...
  $("#room-status").textContent = "Prototype interview active. Speak naturally; essential controls remain available.";
...
function stopPrototypeRoom({ results = true } = {}) {
...
function renderVault() {
  ...
  const sessions = VAULT_SESSIONS.filter((session) => {
  ...
  const transcriptHits = query ? VAULT_SESSIONS.flatMap((session) => session.transcript
  ...
  if (!sessions.length) list.innerHTML = `<p class="empty-state">No saved prototype sessions match that search.</p>`;
}

function openSession(sessionId, time = null) {
  const session = VAULT_SESSIONS.find((item) => item.id === sessionId);
  if (!session) return;
...
function bindStaticEvents() {
  ...
  $("#room-start").addEventListener("click", startPrototypeRoom);
