...
import { account, accountInitials, accountName, loadAccount } from './account.mjs';
...
function renderRail() {
  const nav = NAV.filter((item) => item.route !== 'mentor' || account.identity?.mentor || account.identity?.admin);
  ...
  rail.innerHTML = `
    ...
    <div class="rail-foot">
      <div class="rail-id">
        <div class="rail-avatar">${accountInitials()}</div>
        <div><b>${accountName()}</b><small>${account.identity?.admin ? 'Admin' : account.identity?.mentor ? 'Mentor' : 'Student'}</small></div>
      </div>
      <div class="rail-ent">ENTITLED · IV PREP 360</div>
...
addEventListener('hashchange', () => { if (bootReady) renderRoute(); });
...
loadAccount().then(() => {
  ...
  if (preferences) {
    if (typeof preferences.coachingEnabled === 'boolean') ui.coaching = preferences.coachingEnabled;
    if (typeof preferences.recordingDefault === 'boolean') ui.recording = preferences.recordingDefault;
    if (preferences.visibility && typeof preferences.visibility.analyticsVisible === 'boolean') ui.analyticsVisible = preferences.visibility.analyticsVisible;
    saveUi();
  }
  bootReady = true;
  renderRail();
  ...
  stage.innerHTML = '<div class="screen"><div class="recovery"><div class="t-label" style="color:var(--g-amber)">AUTHENTICATED ACCESS REQUIRED</div><h1 class="t-display-md">IV PREP COULD NOT OPEN.</h1><p class="t-body" style="color:var(--g-mid)">Return to MissionMed Matrix and open IV Prep On-Call again.</p></div></div>';
});
