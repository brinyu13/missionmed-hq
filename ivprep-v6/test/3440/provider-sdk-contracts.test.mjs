...
import { LemonSliceAvatarAdapter } from '../../server/providers/lemonslice-avatar-adapter.mjs';
import { createLiveKitSessionCoordinator } from '../../server/providers/livekit-session-coordinator.mjs';
import { createOpenAiRealtimeModel } from '../../server/providers/openai-realtime-adapter.mjs';
...
test('LemonSlice adapter matches pinned start, readiness, termination, authentication, and status contracts', async () => {
  ...
  class FakeAvatarSession {
    constructor(options) { this.options = options; calls.push(['construct', options]); }
  ...
  assert.deepEqual(created, { sessionId: 'provider-session-1', avatarJoined: true });
...
test('OpenAI Realtime adapter uses exact low reasoning and zero retries', async () => {
  ...
  await createOpenAiRealtimeModel({
    ...
    openaiModule: { realtime: { RealtimeModel } },
  });
...
test('Profile B AgentServer and AgentSession are explicit, terminal on reconnect, and zero retry', { skip: !PINNED_AGENTS_AVAILABLE }, async () => {
  ...
  const avatar = {
    ...
    create: async (options) => { calls.push(['avatar.create', options]); return { sessionId: 'provider-worker-1', avatarJoined: true }; },
    roomOptions: () => ({ outputOptions: { audioEnabled: true } }),
    terminate: async () => { calls.push(['avatar.terminate']); return { confirmed: true }; },
    status: async () => { calls.push(['avatar.status']); return { ok: true, sessionStatus: 'COMPLETED', cost: 1 }; },
  ...
  assert.equal(calls.filter(([name]) => name === 'avatar.status').length, 1);
