...
test('multiple faces suppress every person-specific metric', () => {
  ...
  assert.equal(result.events.some((event)=>event.metric==='multiple_faces_detected'),true);
...
test('facial movement episode has one stable metric schema in the session envelope',()=>{
  ...
  session.beginAnswer({answerId:'a',hasCamera:true});
  for(let at=0;at<=500;at+=125)session.ingestVision({atMs:at,expectedFrameMs:125,geometry:{...compact(1),face:{...compact(1).face,movementRatePerSecond:1}}});
