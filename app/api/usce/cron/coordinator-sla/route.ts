...
import { runCoordinatorSlaJob } from '@/lib/usce/cron/run-coordinator-sla';
import { verifySignedRequest } from '@/lib/usce/security/signed-request';

export async function POST(request: NextRequest) {
  const requestId = createRequestId('usce_cron_coord_sla');
  ...
  const result = await runCoordinatorSlaJob(supabase, 'mirror_endpoint');
