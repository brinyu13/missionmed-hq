...
import { validateApproveOfferBody } from '@/lib/usce/schemas';
...
function requestId(): string {
  return `usce_approve_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 10)}`;
}
...
export async function POST(
  ...
  const parsed = validateApproveOfferBody(payload);
  if (!parsed.success) {
  ...
  const { data: offerRow, error: offerError } = await supabase
    ...
    .from('usce_offers')
    .select('id, request_id, status, preview_subject_body_hash')
    .eq('id', id)
    .maybeSingle();
  ...
  if (!['PREVIEWED', 'APPROVED'].includes(offerRow.status)) {
    return errorResponse(409, {
      code: 'INVALID_TRANSITION',
      message: 'Offer can only be approved from PREVIEWED or APPROVED.',
      requestId: rid,
  ...
  if (!requestRow || requestRow.assigned_coordinator_id !== callerUserId) {
    return errorResponse(403, {
      code: 'FORBIDDEN',
      message: 'Only the assigned coordinator can approve this offer.',
      requestId: rid,
    });
  }

  const bodyHash = await sha256Hex(`${parsed.data.subject}\n${parsed.data.html_body}`);
  if (!offerRow.preview_subject_body_hash || bodyHash !== offerRow.preview_subject_body_hash) {
    return errorResponse(409, {
      code: 'PREVIEW_STALE',
      message: 'Offer body differs from the latest previewed content.',
      requestId: rid,
    });
  }
  ...
  const { data: updated, error: updateError } = await supabase
    ...
    .update({
      ...
      status: 'APPROVED',
      approved_subject_body_hash: bodyHash,
      approved_by: callerUserId,
      approved_at: approvedAt,
    })
    .eq('id', id)
    .select('approved_at, approved_subject_body_hash')
    .single();
  ...
  return NextResponse.json(
    {
      ...
      approved_subject_body_hash_prefix: String(updated.approved_subject_body_hash ?? '').slice(0, 8),
    },
    { status: 200 }
  );
}
