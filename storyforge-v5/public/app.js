...
async function navigate(route, id = null) {
  state.route = route;
  state.routeId = id;
  history.replaceState(null, '', id ? `#${route}/${id}` : `#${route}`);
  try {
    await renderRoute();
  } catch (error) {
...
function renderLogin(errorMessage = '') {
  ...
  document.querySelectorAll('[data-persona]').forEach((button) => {
    button.addEventListener('click', async () => {
      ...
      try {
        const { token } = await request(`/api/dev/session/${button.dataset.persona}`, { method: 'POST', body: '{}' });
        state.token = token;
...
async function init() {
  try {
    ...
    if (state.token) await bootstrapSession();
    else renderLogin();
  } catch (error) {
