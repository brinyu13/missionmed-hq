...
test('student create validates governed relationship and never returns raw email',async()=>{let inserted;const subject=service({identityQuery:async(sql,values)=>{if(sql.includes('sf_story_feature_enabled'))return{rows:[{enabled:true}]};if(sql.includes('sf_request_create')){inserted=values;return{rows:[{payload:{id:promptId,contributor_first_name:'Sam',relationship_id:values[1],email:'sam@example.com',status:'draft',personal_message:'Hello',disclosure_version:'founder-v1',reminders_sent:0,row_version:'0'}}]};}return{rows:[]};}});const result=await subject.create(identity,{recipientFirstName:'Sam',relationship:'medschool_friend',email:'sam@example.com',personalMessage:'Hello'});assert.equal(result.maskedEmail,'s***@example.com');assert.equal(JSON.stringify(result).includes('sam@example.com'),false);assert.equal(inserted[1],'medschool_friend');await assert.rejects(()=>subject.create(identity,{recipientFirstName:'Sam',relationship:'medical_school_friend',email:'sam@example.com'}),RequestsError);await assert.rejects(()=>subject.create(identity,{recipientFirstName:'Sam',relationship:'stranger',email:'sam@example.com'}),RequestsError);});
