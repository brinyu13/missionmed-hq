...
MIGRATION_VERSION="20260806130000"
MIGRATION_FILE="20260806130000_b1_511a_wordpress_admin_authority.sql"
MIGRATION_SHA256="4dfcea71718cfb268bb3b8716b6968df5f678690d0465f941ec2e0501c32f5c1"
EXPECTED_SYSTEM_IDENTIFIER="7667256745042145332"
EXPECTED_USER_COUNT="441"
EXPECTED_ACTIVE_ASSIGNMENT_COUNT="0"

fail() {
  printf 'Refusing B1-511A production migration: %s\n' "$*" >&2
  exit 1
}
...
[[ $# = 1 ]] || fail 'usage: apply-b1-511a-production-migration.sh preflight|apply'
mode="$1"
...
[[ -f "$STORYFORGE_DB_BACKUP_PATH" && ! -L "$STORYFORGE_DB_BACKUP_PATH" ]] \
  || fail 'backup is absent or symlinked'
[[ "$(sha256_file "$STORYFORGE_DB_BACKUP_PATH")" = "$STORYFORGE_DB_BACKUP_SHA256" ]] \
  || fail 'backup hash differs'
...
if [[ "$actual_pre_ledger" != "$expected_pre_ledger" ]]; then
  ...
  [[ "$ledger_without_target" = "$expected_pre_ledger" && "$applied_row" = "$expected_applied" ]] \
    || fail 'production migration ledger differs from the accepted state'
  pending=0
fi
...
[[ "$target_identity" = "railway|postgres|$EXPECTED_SYSTEM_IDENTIFIER|true" ]] \
  || fail 'database target identity differs'
...
[[ "$pre_counts" = "$EXPECTED_USER_COUNT|$EXPECTED_ACTIVE_ASSIGNMENT_COUNT" ]] \
  || fail 'production counts changed after backup'

if [[ "$mode" = preflight ]]; then
  printf 'B1_511A_PRODUCTION_MIGRATION_PREFLIGHT_PASS\n'
  printf 'pending_migrations=%s\n' "$pending"
  exit 0
fi

[[ "${STORYFORGE_MIGRATION_CONFIRM:-}" = B1-511A-APPLY ]] || fail 'apply confirmation is absent'
if [[ "$pending" = 0 ]]; then
  printf 'B1_511A_PRODUCTION_MIGRATION_ALREADY_APPLIED_PASS\n'
  exit 0
fi

{
  printf '%s\n' "SELECT pg_advisory_xact_lock(hashtextextended('missionmed.storyforge.b1-511a.production-migration',0));"
...
DO $b1_511a_post$
BEGIN
  IF (SELECT role FROM public.sf_users WHERE wp_user_id = 1) <> 'student' THEN
    RAISE EXCEPTION 'Founder base role changed';
  END IF;
  IF (SELECT role FROM public.sf_users WHERE wp_user_id = 107) <> 'admin' THEN
    RAISE EXCEPTION 'Additional administrator base role changed';
  END IF;
  IF to_regprocedure('public.sf_actor_wordpress_admin()') IS NULL
     OR to_regprocedure('public.sf_actor_admin_mode()') IS NULL THEN
    RAISE EXCEPTION 'B1-511A authority functions are absent';
  END IF;
END
$b1_511a_post$;
SQL
...
expected_post="$MIGRATION_VERSION|$MIGRATION_FILE|$MIGRATION_SHA256|$STORYFORGE_DEPLOY_GIT_COMMIT|$STORYFORGE_DB_BACKUP_ID"
[[ "$post_row" = "$expected_post" ]] || fail 'post-migration ledger differs'
...
printf 'B1_511A_PRODUCTION_MIGRATION_APPLY_PASS\n'
