...
function fail(message) {
  throw new Error(`StoryForge release provenance failed: ${message}`);
}
...
export function releaseExpectedCommit(environment = process.env) {
  const expectedCommit = String(environment.STORYFORGE_EXPECTED_COMMIT || '').trim();
  if (!fullCommitPattern.test(expectedCommit)) {
    fail('STORYFORGE_EXPECTED_COMMIT must be an explicit full lowercase 40-character commit.');
  }
...
export function assertReleaseSource({
  ...
  if (actualRepositoryDir !== expectedRepositoryDir) {
    fail(
      `repository root ${actualRepositoryDir} is not the StoryForge package parent `
  ...
  if (head !== expectedCommit) {
    fail(`Git HEAD ${head} does not match STORYFORGE_EXPECTED_COMMIT ${expectedCommit}.`);
  }
  ...
  if (
    ...
    fail('the Git index does not exactly match STORYFORGE_EXPECTED_COMMIT.');
  }
