...
test("runtime RLS verifier requires least privilege and hostile two-subject denial", async () => {
  ...
  assert.match(source, /bool_and\(c\.relrowsecurity\) AS "rlsEnabled"/);
  assert.match(source, /bool_and\(c\.relforcerowsecurity\) AS "rlsForced"/);
  ...
  assert.match(source, /crossContributorIdentityDenied: crossIdentityRead\.rowCount === 0/);
  assert.match(source, /publicIntelReadAllowedWithoutIdentity: publicIntelRead\.rowCount === 1/);
  assert.match(source, /studentIntelUpdateDenied: studentIntelUpdate\.rowCount === 0/);
  assert.match(source, /adminContributorIdentityAllowed: adminIdentityRead\.rowCount === 2/);
  assert.match(source, /hiddenIntelReadDenied: hiddenIntelRead\.rowCount === 0/);
