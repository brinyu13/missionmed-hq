...
function mmtlr_handle_consent_post($user) {
    ...
    if ($action === 'grant') {
        ...
        if (!mmtl_withdraw_remote_sync_consent((int) $user->ID)) {
            mmtlr_error(503, 'timeline_consent_withdrawal_failed', 'Secure saving could not be turned off.');
        }
    } else {
        mmtlr_error(400, 'timeline_consent_action_invalid', 'Choose a valid secure-saving action.');
    }
...
function mmtlr_serve() {
    ...
    if (!function_exists('mmtl_eligibility_state')) {
    ...
    if ($method === 'POST') {
        mmtlr_handle_consent_post(wp_get_current_user());
    }
    $bundle = mmtlr_bundle();
