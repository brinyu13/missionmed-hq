...
try{
 ...
 const reviewed=await adminService.roster(admin,{wpUserIds:[actors.other.wp],verifiedAt:new Date().toISOString()});check('Roster ignores forged mutable Guardian readiness and derives a conservative MissionMed rule result without returning private source',reviewed.students[0].exportReadiness==='NEEDS_REVIEW'&&reviewed.students[0].guardianBasis==='MISSIONMED_RULE'&&!JSON.stringify(reviewed).includes('quality_source')&&!JSON.stringify(reviewed).includes('Clinical Research Coordinator'));
 ...
 check('Export bookkeeping cannot promote the conservative MissionMed rule result',(await adminService.roster(admin,{wpUserIds:[actors.other.wp],verifiedAt:new Date().toISOString()})).students[0].exportReadiness==='NEEDS_REVIEW');
 ...
 check('Changed factual content receives a fresh conservative MissionMed rule result',await adminService.roster(admin,{wpUserIds:[actors.other.wp],verifiedAt:new Date().toISOString()}).then(value=>value.students[0].exportReadiness==='NEEDS_REVIEW'&&value.students[0].guardianBasis==='MISSIONMED_RULE'));
 await adminService.open(admin,{wpUserId:actors.student.wp,verifiedAt:new Date().toISOString()});
 command('psql',['-h',temporary,'-p','57423','-U','d1_022_owner','-d','postgres','-v','ON_ERROR_STOP=1','-f',join(root,'database/migrations/20260908015000_d1_022_current_document_version_scope.down.sql')]);
 command('psql',['-h',temporary,'-p','57423','-U','d1_022_owner','-d','postgres','-v','ON_ERROR_STOP=1','-f',join(root,'database/migrations/20260908014000_d1_022_version_history_scope.down.sql')]);
 command('psql',['-h',temporary,'-p','57423','-U','d1_022_owner','-d','postgres','-v','ON_ERROR_STOP=1','-f',join(root,'database/migrations/20260907013000_d1_022_admin_outbox.down.sql')]);
