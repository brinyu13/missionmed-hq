...
function cacheName(origin,principalId,resourceId){
  const environment=new URL(origin).hostname.toLowerCase().replace(/[^a-z0-9.-]/g,"-");
  return `missionmed-timeline:${environment}:principal:${principalId}:resource:${resourceId}:v1`;
}
...
export async function prepareTimelineProductionRuntime({fetchImpl=globalThis.fetch.bind(globalThis),locationObject=globalThis.location}={}){
  ...
  const authClient=new TimelineProductionAuthClient({
    ...
    onAccountSwitch:()=>{
      adapter?.close?.();
      globalThis.dispatchEvent?.(new CustomEvent("mission-timeline-account-changed"));
  ...
  const resourceId=String(active?.document?.id||"unassigned");
  adapter=new HybridIndexedDbAdapter({
    apiClient:authClient,
    programId:String(active?.document?.programId||"missionmed-360:3893"),
    name:cacheName(locationObject.origin,identity.principalId,resourceId),
    version:1,
  ...
  if(active?.document){
    ...
    await adapter.hydrateAuthoritative([
